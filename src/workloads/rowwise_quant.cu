...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--warmup") {
            ...
            std::cout
                << "Usage: rowwise_quant_bench [--csv] [--warmup N] "
...
double modeled_bytes(int rows, int cols) {
    ...
    return elements * (2.0 * sizeof(float) + sizeof(std::int8_t)) +
...
__device__ __forceinline__ float cluster_reduce_max(cg::cluster_group cluster,
                                                     float block_value) {
    __shared__ float cluster_values[MaxClusterSize];
    __shared__ float broadcast;
    ...
    if (threadIdx.x < blocks_per_cluster) {
        float* remote_value =
            cluster.map_shared_rank(&cluster_values[block_rank], threadIdx.x);
        *remote_value = block_value;
    }
    cluster.sync();

    float reduced = 0.0f;
    if (threadIdx.x < blocks_per_cluster) {
        reduced = cluster_values[threadIdx.x];
    }
    if (threadIdx.x < 32) {
        reduced = subwarp_reduce_max<32>(reduced);
    }
...
__host__ __device__ __forceinline__ std::int8_t quantize_to_i8(float value,
                                                               float inv_scale) {
    float q = nearbyintf(value * inv_scale);
    q = fminf(kQMax, fmaxf(-kQMax, q));
...
__global__ void init_input_kernel(float* data, std::size_t elements) {
    ...
    const float base = static_cast<float>(a & 4095u) * (1.0f / 2048.0f) - 1.0f;
    const float tiny = static_cast<float>(b & 255u) * 1.0e-7f;
    data[idx] = base + tiny;
}

template <int BlockThreads>
__global__ void block_quant_read_kernel(const float* __restrict__ input,
    ...
    const float max_abs = block_reduce_max<BlockThreads>(local_max);
    const float inv_scale = max_abs > 0.0f ? kQMax / max_abs : 0.0f;
    if (row_valid && threadIdx.x == 0) {
        scales[row] = max_abs / kQMax;
    }
    if (!row_valid) {
        return;
    }
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        output[row_base + col] = quantize_to_i8(input[row_base + col], inv_scale);
    }
}

template <int BlockThreads>
__global__ void block_quant_staged_kernel(const float* __restrict__ input,
                                          std::int8_t* __restrict__ output,
                                          float* __restrict__ scales,
                                          int rows,
                                          int cols) {
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        tile[col] = row_valid ? input[row_base + col] : 0.0f;
    }
    __syncthreads();

    float local_max = 0.0f;
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        local_max = fmaxf(local_max, fabsf(tile[col]));
    }
    const float max_abs = block_reduce_max<BlockThreads>(local_max);
    const float inv_scale = max_abs > 0.0f ? kQMax / max_abs : 0.0f;
    if (row_valid && threadIdx.x == 0) {
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        output[row_base + col] = quantize_to_i8(tile[col], inv_scale);
    }
}

template <int BlockThreads, int MaxClusterSize>
__global__ void cluster_quant_staged_kernel(const float* __restrict__ input,
                                            std::int8_t* __restrict__ output,
                                            float* __restrict__ scales,
                                            int rows,
                                            int cols,
    ...
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        tile[col] = input[row_base + slice_start + col];
    }
    __syncthreads();
    ...
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        output[row_base + slice_start + col] =
            quantize_to_i8(tile[col], inv_scale);
    }
    cluster.sync();
}
...
DeviceInfo query_device_info(const Options& options) {
    ...
    if (info.cluster_launch) {
        ...
        CUDA_CHECK(cudaOccupancyMaxPotentialClusterSize(
            &max_cluster_size,
            cluster_quant_staged_kernel<kBlockThreads, kMaxClusterSize>,
            &config));
...
VariantResult benchmark_block_read(const Options& options,
    ...
    auto launch = [&]() {
        block_quant_read_kernel<kBlockThreads>
            <<<rows, kBlockThreads>>>(d_input, d_output, d_scales, rows, cols);
    };
...
VariantResult benchmark_block_staged(const Options& options,
    ...
    CUDA_CHECK(cudaFuncSetAttribute(
        block_quant_staged_kernel<kBlockThreads>,
    ...
    auto launch = [&]() {
        block_quant_staged_kernel<kBlockThreads>
            <<<rows, kBlockThreads, smem_bytes>>>(d_input, d_output, d_scales,
                                                  rows, cols);
    };
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
...
VariantResult benchmark_best_block(const Options& options,
                                   ...
                                   int cols) {
    VariantResult best;
    best = pick_better(best, benchmark_block_read(options, d_input, d_output,
                                                  d_scales, rows, cols));
    best = pick_better(best, benchmark_block_staged(options, device, d_input,
                                                    d_output, d_scales, rows,
                                                    cols));
    return best;
}
...
VariantResult benchmark_cluster(const Options& options,
    ...
    auto launch = [&]() {
        ...
        CUDA_CHECK(cudaLaunchKernelEx(
            &config,
            cluster_quant_staged_kernel<kBlockThreads, kMaxClusterSize>,
            d_input, d_output, d_scales, rows, cols, slice_elems));
    };
    result.valid = true;
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
...
void verify_output_samples(const float* d_input,
    ...
    std::vector<std::int8_t> h_output(cols);
    std::vector<float> h_scales(rows);
    CUDA_CHECK(cudaMemcpy(h_scales.data(), d_scales,
    ...
    for (int row : sample_rows) {
        ...
        for (int col = 0; col < cols; ++col) {
            max_abs = std::max(max_abs, std::fabs(h_input[col]));
        }
        const float expected_scale = max_abs / kQMax;
        const float inv_scale = max_abs > 0.0f ? kQMax / max_abs : 0.0f;
        const float scale_err = std::fabs(h_scales[row] - expected_scale);
        if (scale_err > 1.0e-6f) {
            std::ostringstream oss;
            oss << label << " scale verification failed row=" << row
                << " got=" << h_scales[row] << " ref=" << expected_scale
                << " err=" << scale_err;
            throw std::runtime_error(oss.str());
        }
        for (int col : sample_cols) {
            const std::int8_t expected = quantize_to_i8(h_input[col], inv_scale);
            if (h_output[col] != expected) {
                std::ostringstream oss;
                oss << label << " q verification failed row=" << row
                    << " col=" << col << " got="
                    << static_cast<int>(h_output[col]) << " ref="
...
void verify_selected(const Options& options,
    ...
    if (block.valid && options.profile_variant != "cluster") {
        if (block.name == "block_smem") {
            benchmark_block_staged(one_run, device, d_input, d_output, d_scales,
                                   rows, cols);
        } else {
            benchmark_block_read(one_run, d_input, d_output, d_scales, rows, cols);
        }
        verify_output_samples(d_input, d_output, d_scales, rows, cols, "block");
    }
    if (cluster.valid && options.profile_variant != "block") {
        benchmark_cluster(one_run, device, d_input, d_output, d_scales, rows, cols);
        verify_output_samples(d_input, d_output, d_scales, rows, cols, "cluster");
    }
}
...
void run_shape(int cols, const Options& options, const DeviceInfo& device) {
    ...
    std::int8_t* d_output = nullptr;
    float* d_scales = nullptr;
    CUDA_CHECK(cudaMalloc(&d_input, elements * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_output, elements * sizeof(std::int8_t)));
    CUDA_CHECK(cudaMalloc(&d_scales, static_cast<std::size_t>(rows) * sizeof(float)));
    ...
    if (options.profile_variant != "cluster") {
        block = benchmark_best_block(options, device, d_input, d_output, d_scales,
                                     rows, cols);
    }
    if (options.profile_variant != "block") {
        cluster =
            benchmark_cluster(options, device, d_input, d_output, d_scales, rows,
                              cols);
    }
    verify_selected(options, device, block, cluster, d_input, d_output, d_scales,
                    rows, cols);
