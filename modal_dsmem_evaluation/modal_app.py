...
REMOTE_ROOT = "/opt/modal_dsmem_evaluation"
...
app = modal.App("dsmem-cross-gpu-evaluation")
...
def _remote_run(requested_gpu: str, config: dict[str, Any]) -> dict[str, Any]:
    import torch

    from dsmem_eval.runner import run_suite

    device_name = torch.cuda.get_device_name(0)
    expected = "H100" if requested_gpu == "H100!" else "B200"
    if expected not in device_name:
        raise RuntimeError(
            f"Modal allocated {device_name!r} for {requested_gpu!r}; "
            f"refusing to record it as {expected}"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="dsmem-eval-") as temporary:
        ...
        try:
            metadata = run_suite(config, output_dir, requested_gpu)
        ...
        slug = f"{expected.lower()}_{timestamp}"
        return {
            "slug": slug,
            "metadata": metadata,
...
def run_h100(config: dict[str, Any]) -> dict[str, Any]:
    return _remote_run("H100!", config)
...
def run_b200(config: dict[str, Any]) -> dict[str, Any]:
    return _remote_run("B200", config)
...
def main(
    ...
    gpu = gpu.lower().strip()
    if gpu not in {"h100", "b200", "all"}:
        raise ValueError("--gpu must be h100, b200, or all")
    workload_names = (
        [
            "layernorm_backward",
    ...
    functions = []
    if gpu in {"h100", "all"}:
        functions.append(("H100", run_h100))
    if gpu in {"b200", "all"}:
        functions.append(("B200", run_b200))

    for label, function in functions:
        print(f"Starting {label} evaluation", flush=True)
