...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--warmup") {
            ...
            std::cout
                << "Usage: gated_standardize_bench [--csv] [--warmup N] "
...
__host__ __device__ __forceinline__ float sigmoid_value(float value) {
...
__host__ __device__ __forceinline__ float normalize_value(float value,
    ...
    return norm * sigmoid_value(gate);
}
...
__global__ void block_norm_read_kernel(const float* __restrict__ input,
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        output[row_base + col] =
            normalize_value(input[row_base + col], gate[row_base + col],
                            row_stats, cols);
    }
}
...
__global__ void block_norm_staged_kernel(const float* __restrict__ input,
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        output[row_base + col] =
            normalize_value(tile_x[col], tile_gate[col], row_stats, cols);
    }
}
...
__global__ void cluster_norm_staged_kernel(const float* __restrict__ input,
    ...
    extern __shared__ float tile[];
    float* tile_x = tile;
    float* tile_gate = tile + slice_elems;
...
VariantResult benchmark_block_read(const Options& options,
    ...
    auto launch = [&]() {
        block_norm_read_kernel<kBlockThreads>
            <<<rows, kBlockThreads>>>(d_input, d_gate, d_output, rows, cols);
    };
...
VariantResult benchmark_block_staged(const Options& options,
    ...
    auto launch = [&]() {
        block_norm_staged_kernel<kBlockThreads>
            <<<rows, kBlockThreads, smem_bytes>>>(d_input, d_gate, d_output,
                                                  rows, cols);
    };
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
...
VariantResult benchmark_best_block(const Options& options,
    ...
    best = pick_better(best,
                       benchmark_block_read(options, d_input, d_gate, d_output,
                                            rows, cols));
    best = pick_better(best,
                       benchmark_block_staged(options, device, d_input,
                                              d_gate, d_output, rows, cols));
    return best;
}
...
VariantResult benchmark_cluster(const Options& options,
    ...
    auto launch = [&]() {
        ...
        CUDA_CHECK(cudaLaunchKernelEx(
            &config,
            cluster_norm_staged_kernel<kBlockThreads, kMaxClusterSize>,
            d_input, d_gate, d_output, rows, cols, slice_elems));
    };
    result.valid = true;
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
    ...
    for (int row : sample_rows) {
        ...
        for (int col : sample_cols) {
            ...
            const double sigmoid =
                1.0 / (1.0 + std::exp(-static_cast<double>(h_gate[col])));
            const float expected = static_cast<float>(norm * sigmoid);
...
void verify_selected(const Options& options,
    ...
    if (block.valid && options.profile_variant != "cluster") {
        if (block.name == "block_smem") {
            benchmark_block_staged(one_run, device, d_input, d_gate, d_output,
                                   rows, cols);
        } else {
            benchmark_block_read(one_run, d_input, d_gate, d_output, rows,
                                 cols);
        }
        verify_output_samples(d_input, d_gate, d_output, rows, cols, "block");
    }
    if (cluster.valid && options.profile_variant != "block") {
        benchmark_cluster(one_run, device, d_input, d_gate, d_output, rows,
                          cols);
        verify_output_samples(d_input, d_gate, d_output, rows, cols,
                              "cluster");
    }
}
...
void run_shape(int cols, const Options& options, const DeviceInfo& device) {
    ...
    if (options.profile_variant != "cluster") {
        block = benchmark_best_block(options, device, d_input, d_gate, d_output,
                                     rows, cols);
    }
    if (options.profile_variant != "block") {
        cluster =
            benchmark_cluster(options, device, d_input, d_gate, d_output, rows,
                              cols);
    }
    verify_selected(options, device, block, cluster, d_input, d_gate, d_output,
                    rows, cols);
