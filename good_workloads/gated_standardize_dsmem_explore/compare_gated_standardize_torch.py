...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare row-wise gated standardization DSMEM kernels with torch.compile."
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_gated_standardize.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def gated_standardize(x: torch.Tensor, gate: torch.Tensor) -> torch.Tensor:
    mean = x.mean(dim=-1, keepdim=True)
    mean_sq = (x * x).mean(dim=-1, keepdim=True)
    inv_std = torch.rsqrt(torch.clamp(mean_sq - mean * mean, min=0.0) + EPS)
    return (x - mean) * inv_std * torch.sigmoid(gate)
...
def benchmark_torch_compile(
    ...
    gate = make_input(rows, cols, device)
    torch._dynamo.reset()
    compiled = torch.compile(gated_standardize)
    compiled(x, gate)
    torch.cuda.synchronize()
    avg_ms = time_callable(lambda: compiled(x, gate), warmup, iters)
    del x, gate
    torch._dynamo.reset()
...
def main() -> int:
    ...
    print(
        f"Row-wise gated standardization, rows={args.rows}, "
