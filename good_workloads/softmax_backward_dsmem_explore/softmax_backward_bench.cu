...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--warmup") {
            ...
            std::cout
                << "Usage: softmax_backward_bench [--csv] [--warmup N] "
...
__global__ void init_input_kernel(float* y, float* dy, std::size_t elements) {
    ...
    y[idx] = 0.0001f + static_cast<float>(a & 4095u) * (1.0f / 4096.0f);
    dy[idx] = static_cast<float>(b & 2047u) * (1.0f / 1024.0f) - 1.0f;
}

template <int BlockThreads>
__global__ void block_softmax_bwd_read_kernel(const float* __restrict__ y,
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        dot += y[row_base + col] * dy[row_base + col];
    }
    const float row_dot = block_reduce_sum<BlockThreads>(dot);
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        dx[row_base + col] = yv * (dyv - row_dot);
    }
}

template <int BlockThreads>
__global__ void block_softmax_bwd_staged_kernel(const float* __restrict__ y,
                                                const float* __restrict__ dy,
                                                float* __restrict__ dx,
                                                int rows,
                                                int cols) {
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        dx[row_base + col] = tile_y[col] * (tile_dy[col] - row_dot);
    }
}

template <int BlockThreads, int MaxClusterSize>
__global__ void cluster_softmax_bwd_staged_kernel(const float* __restrict__ y,
                                                  const float* __restrict__ dy,
                                                  float* __restrict__ dx,
                                                  int rows,
                                                  int cols,
    ...
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        ...
        tile_y[col] = y[row_base + global_col];
    ...
    const float block_dot = block_reduce_sum<BlockThreads>(dot);
    const float row_dot =
    ...
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        const int global_col = slice_start + col;
        dx[row_base + global_col] = tile_y[col] * (tile_dy[col] - row_dot);
    }
}
...
DeviceInfo query_device_info(const Options& options) {
    ...
    if (info.cluster_launch) {
        ...
        CUDA_CHECK(cudaOccupancyMaxPotentialClusterSize(
            &max_cluster_size,
            cluster_softmax_bwd_staged_kernel<kBlockThreads, kMaxClusterSize>,
            &config));
...
VariantResult benchmark_block_read(const Options& options,
    ...
    auto launch = [&]() {
        block_softmax_bwd_read_kernel<kBlockThreads>
            <<<rows, kBlockThreads>>>(d_y, d_dy, d_dx, rows, cols);
    };
...
VariantResult benchmark_block_staged(const Options& options,
    ...
    CUDA_CHECK(cudaFuncSetAttribute(
        block_softmax_bwd_staged_kernel<kBlockThreads>,
    ...
    auto launch = [&]() {
        block_softmax_bwd_staged_kernel<kBlockThreads>
            <<<rows, kBlockThreads, smem_bytes>>>(d_y, d_dy, d_dx, rows, cols);
    };
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
    return result;
...
VariantResult benchmark_best_block(const Options& options,
    ...
    best = pick_better(best,
                       benchmark_block_read(options, d_y, d_dy, d_dx, rows,
                                            cols));
    best = pick_better(best,
                       benchmark_block_staged(options, device, d_y, d_dy, d_dx,
                                              rows, cols));
    return best;
}
...
VariantResult benchmark_cluster(const Options& options,
    ...
    auto launch = [&]() {
        ...
        CUDA_CHECK(cudaLaunchKernelEx(
            &config,
            cluster_softmax_bwd_staged_kernel<kBlockThreads, kMaxClusterSize>,
            d_y, d_dy, d_dx, rows, cols, slice_elems));
    };
    result.valid = true;
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
...
void verify_output_samples(const float* d_y,
                           const float* d_dy,
                           const float* d_dx,
                           int rows,
                           int cols,
    ...
    for (int row : sample_rows) {
        ...
        for (int col = 0; col < cols; ++col) {
            dot += static_cast<double>(h_y[col]) *
                   static_cast<double>(h_dy[col]);
        }

        for (int col : sample_cols) {
            const double ref = static_cast<double>(h_y[col]) *
                               (static_cast<double>(h_dy[col]) - dot);
            float got = 0.0f;
...
void verify_selected(const Options& options,
    ...
    if (block.valid && options.profile_variant != "cluster") {
        if (block.name == "block_smem") {
            benchmark_block_staged(one_run, device, d_y, d_dy, d_dx, rows, cols);
        } else {
            benchmark_block_read(one_run, d_y, d_dy, d_dx, rows, cols);
        }
        verify_output_samples(d_y, d_dy, d_dx, rows, cols, "block");
    }
    if (cluster.valid && options.profile_variant != "block") {
        benchmark_cluster(one_run, device, d_y, d_dy, d_dx, rows, cols);
        verify_output_samples(d_y, d_dy, d_dx, rows, cols, "cluster");
    }
}
...
void run_shape(int cols, const Options& options, const DeviceInfo& device) {
    ...
    if (options.profile_variant != "cluster") {
        block = benchmark_best_block(options, device, d_y, d_dy, d_dx, rows, cols);
    }
    if (options.profile_variant != "block") {
        cluster = benchmark_cluster(options, device, d_y, d_dy, d_dx, rows, cols);
    }
    verify_selected(options, device, block, cluster, d_y, d_dy, d_dx, rows, cols);
