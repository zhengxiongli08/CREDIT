...
ROWS = int(os.environ.get("ROWS", "8192"))
...
def summarize(variant: str, path: Path) -> dict[str, float | int | str] | None:
    ...
    modeled_bytes = elements * 4.0 * 5.0
    return {
        "variant": variant,
        "cols": COLS,
        "rows": ROWS,
...
def main() -> int:
    cases = [
        ("block", RESULTS / "ncu_softmax_backward_block.csv"),
        ("cluster", RESULTS / "ncu_softmax_backward_cluster.csv"),
    ]
    rows = []
    ...
    out = RESULTS / "softmax_backward_ncu_summary.csv"
    fieldnames = [
        "variant",
        "cols",
        "rows",
