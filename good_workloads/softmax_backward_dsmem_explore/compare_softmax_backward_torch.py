...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare softmax backward DSMEM kernels with torch.compile."
    )
    parser.add_argument("--rows", "--batch-size", type=int, default=8192)
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "softmax_backward_torch_compare.csv",
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_softmax_backward.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def softmax_backward_dx(y: torch.Tensor, dy: torch.Tensor) -> torch.Tensor:
    dot = (y * dy).sum(dim=-1, keepdim=True)
    return y * (dy - dot)
...
def benchmark_torch_compile(
    ...
    y, dy = make_inputs(rows, cols, device)
    torch._dynamo.reset()
    compiled = torch.compile(softmax_backward_dx)
    compiled(y, dy)
    torch.cuda.synchronize()
    avg_ms = time_callable(lambda: compiled(y, dy), warmup, iters)
    del y, dy
    torch._dynamo.reset()
...
def main() -> int:
    ...
    print(
        f"softmax backward dx, rows={args.rows}, warmup={args.warmup}, "
