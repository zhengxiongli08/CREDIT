...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare cosine-similarity backward DSMEM kernels with torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "cosine_backward_torch_compare.csv",
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_cosine_backward.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def cosine_bwd(
    ...
    inv_x = torch.rsqrt(xx + EPS)
    inv_y = torch.rsqrt(yy + EPS)
    inv_xy = inv_x * inv_y
    coeff_x = dot * inv_x * inv_x * inv_x * inv_y
    coeff_y = dot * inv_x * inv_y * inv_y * inv_y
...
def benchmark_torch_compile(
    ...
    torch._dynamo.reset()
    compiled = torch.compile(cosine_bwd)
    compiled(x, y, grad)
    torch.cuda.synchronize()
...
def main() -> int:
    ...
    print(
        f"Cosine similarity backward, rows={args.rows}, warmup={args.warmup}, "
