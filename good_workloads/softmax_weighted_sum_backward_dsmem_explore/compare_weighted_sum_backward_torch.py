...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare softmax weighted sum backward DSMEM kernels with torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "weighted_sum_backward_torch_compare.csv",
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_weighted_sum_backward.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def make_inputs(
    ...
    values = torch.empty((rows, cols), device=device, dtype=torch.float32)
    dout = torch.empty((rows,), device=device, dtype=torch.float32)
    scores.uniform_(-4.0, 4.0)
    values.uniform_(-1.0, 1.0)
    dout.uniform_(-1.0, 1.0)
    return scores, values, dout
...
def weighted_sum_backward(
    scores: torch.Tensor, values: torch.Tensor, dout: torch.Tensor
) -> tuple[torch.Tensor, torch.Tensor]:
    probs = torch.softmax(scores, dim=-1)
    output = (probs * values).sum(dim=-1, keepdim=True)
    scale = dout[:, None]
    dscores = scale * probs * (values - output)
    dvalues = scale * probs
...
def benchmark_torch_compile(
    ...
    scores, values, dout = make_inputs(rows, cols, device)
    torch._dynamo.reset()
    compiled = torch.compile(weighted_sum_backward)
    compiled(scores, values, dout)
    torch.cuda.synchronize()
    avg_ms = time_callable(lambda: compiled(scores, values, dout), warmup, iters)
    del scores, values, dout
    torch._dynamo.reset()
...
def main() -> int:
    ...
    print(
        f"softmax weighted sum backward, rows={args.rows}, warmup={args.warmup}, "
