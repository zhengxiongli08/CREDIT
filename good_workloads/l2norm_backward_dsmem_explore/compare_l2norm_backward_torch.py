...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare L2 normalization backward dx DSMEM kernels with torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "l2norm_backward_torch_compare.csv",
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_l2norm_backward.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def l2norm_bwd_dx(x: torch.Tensor, dy: torch.Tensor) -> torch.Tensor:
    norm2 = (x * x).sum(dim=-1, keepdim=True)
    dot = (dy * x).sum(dim=-1, keepdim=True)
    inv_norm = torch.rsqrt(norm2 + EPS)
    return dy * inv_norm - x * dot * inv_norm * inv_norm * inv_norm
...
def benchmark_torch_compile(
    ...
    x, dy = make_inputs(rows, cols, device)
    torch._dynamo.reset()
    compiled = torch.compile(l2norm_bwd_dx)
    compiled(x, dy)
    torch.cuda.synchronize()
    avg_ms = time_callable(lambda: compiled(x, dy), warmup, iters)
    del x, dy
    torch._dynamo.reset()
...
def main() -> int:
    ...
    print(
        f"L2 normalization backward dx, rows={args.rows}, warmup={args.warmup}, "
