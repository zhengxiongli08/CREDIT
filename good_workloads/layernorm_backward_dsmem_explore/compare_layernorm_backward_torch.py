...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare LayerNorm backward dx DSMEM kernels with torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "layernorm_backward_torch_compare.csv",
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_layernorm_backward.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def layernorm_bwd_dx(x: torch.Tensor, dy: torch.Tensor, gamma: torch.Tensor) -> torch.Tensor:
...
def benchmark_torch_compile(
    ...
    torch._dynamo.reset()
    compiled = torch.compile(layernorm_bwd_dx)
    compiled(x, dy, gamma)
    torch.cuda.synchronize()
...
def main() -> int:
    ...
    print(
        f"LayerNorm backward dx, rows={args.rows}, warmup={args.warmup}, "
