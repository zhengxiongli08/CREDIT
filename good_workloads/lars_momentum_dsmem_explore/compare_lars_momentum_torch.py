...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare LARS momentum DSMEM kernels with torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "lars_momentum_torch_compare.csv",
    )
...
def lars_momentum_update(
    ...
    update = MOMENTUM * momentum + grad + WEIGHT_DECAY * weight
...
def benchmark_torch_compile(
    ...
    torch._dynamo.reset()
    compiled = torch.compile(lars_momentum_update)
...
def main() -> int:
    ...
    print(
        f"LARS momentum update, rows={args.rows}, warmup={args.warmup}, "
