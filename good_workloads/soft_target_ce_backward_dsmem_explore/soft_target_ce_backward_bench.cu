...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--warmup") {
            ...
            std::cout
                << "Usage: soft_target_ce_backward_bench [--csv] [--warmup N] "
...
__global__ void init_input_kernel(float* logits,
    ...
    if (idx < elements) {
        ...
        logits[idx] = (static_cast<float>(a & 4095u) - 2048.0f) *
                      (1.0f / 512.0f);
        target_probs[idx] = 1.0f / static_cast<float>(cols);
    }
}

template <int BlockThreads>
__global__ void block_soft_target_ce_bwd_read_kernel(
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        dx[row_base + col] = p - target_probs[row_base + col];
    }
}

template <int BlockThreads>
__global__ void block_soft_target_ce_bwd_staged_kernel(
    const float* __restrict__ logits,
    const float* __restrict__ target_probs,
    float* __restrict__ dx,
    int rows,
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        tile_targets[col] = row_valid ? target_probs[row_base + col] : 0.0f;
    }
    __syncthreads();
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        dx[row_base + col] = p - tile_targets[col];
    }
}

template <int BlockThreads, int MaxClusterSize>
__global__ void cluster_soft_target_ce_bwd_staged_kernel(
    const float* __restrict__ logits,
    const float* __restrict__ target_probs,
    float* __restrict__ dx,
    int rows,
    ...
    extern __shared__ float tile[];
    float* tile_logits = tile;
    float* tile_targets = tile + slice_elems;
    ...
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        ...
        const std::size_t idx = row_base + global_col;
        tile_logits[col] = logits[idx];
        tile_targets[col] = target_probs[idx];
    }
    __syncthreads();
    ...
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        const int global_col = slice_start + col;
        const float p = expf(tile_logits[col] - row_max) * inv_sum;
        dx[row_base + global_col] = p - tile_targets[col];
    }
}
...
DeviceInfo query_device_info(const Options& options) {
    ...
    if (info.cluster_launch) {
        ...
        CUDA_CHECK(cudaOccupancyMaxPotentialClusterSize(
            &max_cluster_size,
            cluster_soft_target_ce_bwd_staged_kernel<kBlockThreads,
                                                     kMaxClusterSize>,
            &config));
...
VariantResult benchmark_block_read(const Options& options,
    ...
    auto launch = [&]() {
        block_soft_target_ce_bwd_read_kernel<kBlockThreads>
            <<<rows, kBlockThreads>>>(d_logits, d_target_probs, d_dx, rows,
                                      cols);
    };
...
VariantResult benchmark_block_staged(const Options& options,
    ...
    CUDA_CHECK(cudaFuncSetAttribute(
        block_soft_target_ce_bwd_staged_kernel<kBlockThreads>,
    ...
    auto launch = [&]() {
        block_soft_target_ce_bwd_staged_kernel<kBlockThreads>
            <<<rows, kBlockThreads, smem_bytes>>>(d_logits, d_target_probs,
                                                  d_dx, rows, cols);
    };
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
...
VariantResult benchmark_best_block(const Options& options,
    ...
    best = pick_better(best,
                       benchmark_block_read(options, d_logits, d_target_probs,
                                            d_dx, rows, cols));
    best = pick_better(best,
                       benchmark_block_staged(options, device, d_logits,
                                              d_target_probs, d_dx, rows,
                                              cols));
    return best;
}
...
VariantResult benchmark_cluster(const Options& options,
    ...
    CUDA_CHECK(cudaFuncSetAttribute(
        cluster_soft_target_ce_bwd_staged_kernel<kBlockThreads, kMaxClusterSize>,
        cudaFuncAttributeMaxDynamicSharedMemorySize,
        static_cast<int>(smem_bytes)));
    auto launch = [&]() {
        cudaLaunchConfig_t config{};
        ...
        CUDA_CHECK(cudaLaunchKernelEx(
            ...
            cluster_soft_target_ce_bwd_staged_kernel<kBlockThreads,
                                                     kMaxClusterSize>,
            d_logits, d_target_probs, d_dx, rows, cols, slice_elems));
    };
    result.valid = true;
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
...
void verify_output_samples(const float* d_logits,
    ...
    std::vector<float> h_targets(cols);
    ...
    for (int row : sample_rows) {
        ...
        CUDA_CHECK(cudaMemcpy(h_targets.data(), d_target_probs + row_base,
                              static_cast<std::size_t>(cols) * sizeof(float),
                              cudaMemcpyDeviceToHost));
        ...
        for (int col : sample_cols) {
            ...
            const double ref = p - static_cast<double>(h_targets[col]);
            float got = 0.0f;
            ...
            if (err > 2e-2 + 2e-2 * std::abs(ref)) {
                ...
                oss << label << " verification failed row=" << row
                    << " col=" << col << " target=" << h_targets[col] << " got=" << got
                    << " ref=" << ref << " err=" << err;
...
void verify_selected(const Options& options,
    ...
    if (block.valid && options.profile_variant != "cluster") {
        if (block.name == "block_smem") {
            benchmark_block_staged(one_run, device, d_logits, d_target_probs,
                                   d_dx, rows, cols);
        } else {
            benchmark_block_read(one_run, d_logits, d_target_probs, d_dx, rows,
                                 cols);
        }
        verify_output_samples(d_logits, d_target_probs, d_dx, rows, cols,
                              "block");
    }
    if (cluster.valid && options.profile_variant != "block") {
        benchmark_cluster(one_run, device, d_logits, d_target_probs, d_dx,
                          rows, cols);
        verify_output_samples(d_logits, d_target_probs, d_dx, rows, cols,
                              "cluster");
    }
...
void run_shape(int cols, const Options& options, const DeviceInfo& device) {
    ...
    float* d_target_probs = nullptr;
    float* d_dx = nullptr;
    CUDA_CHECK(cudaMalloc(&d_logits, elements * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_target_probs, elements * sizeof(float)));
    ...
    init_input_kernel<<<(elements + init_threads - 1) / init_threads, init_threads>>>(
        d_logits, d_target_probs, elements, cols);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    if (options.profile_variant != "cluster") {
        block = benchmark_best_block(options, device, d_logits, d_target_probs,
                                     d_dx, rows, cols);
    }
    if (options.profile_variant != "block") {
        cluster = benchmark_cluster(options, device, d_logits, d_target_probs,
                                    d_dx, rows, cols);
    }
    verify_selected(options, device, block, cluster, d_logits, d_target_probs,
                    d_dx, rows, cols);
    print_result(cols, rows, block, cluster, device, options.csv);

    CUDA_CHECK(cudaFree(d_logits));
    CUDA_CHECK(cudaFree(d_target_probs));
