...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare soft-target cross entropy backward DSMEM kernels with torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "soft_target_ce_backward_torch_compare.csv",
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_soft_target_ce_backward.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def make_inputs(rows: int, cols: int, device: torch.device) -> tuple[torch.Tensor, torch.Tensor]:
    logits = torch.empty((rows, cols), device=device, dtype=torch.float32)
    logits.uniform_(-4.0, 4.0)
    target_probs = torch.full((rows, cols), 1.0 / cols, device=device, dtype=torch.float32)
    return logits, target_probs


def modeled_bytes(rows: int, cols: int) -> int:
    return rows * cols * 4 * 5
...
def soft_target_ce_backward_dx(
    logits: torch.Tensor, target_probs: torch.Tensor
) -> torch.Tensor:
    return torch.softmax(logits, dim=-1) - target_probs
...
def benchmark_torch_compile(
    ...
    logits, target_probs = make_inputs(rows, cols, device)
    torch._dynamo.reset()
    compiled = torch.compile(soft_target_ce_backward_dx)
    compiled(logits, target_probs)
    torch.cuda.synchronize()
    avg_ms = time_callable(lambda: compiled(logits, target_probs), warmup, iters)
    del logits, target_probs
    torch._dynamo.reset()
...
def main() -> int:
    ...
    print(
        f"soft-target CE backward from logits, rows={args.rows}, warmup={args.warmup}, "
