...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare triplet-distance backward DSMEM kernels with torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "triplet_distance_backward_torch_compare.csv",
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_triplet_distance_backward.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def make_inputs(
    ...
    z = torch.empty((rows, cols), device=device, dtype=torch.float32)
    grad = torch.empty((rows, 1), device=device, dtype=torch.float32)
    x.uniform_(-1.0, 1.0)
    y.uniform_(-1.0, 1.0)
    z.uniform_(-1.0, 1.0)
    grad.uniform_(0.75, 1.25)
    return x, y, z, grad


def modeled_bytes(rows: int, cols: int) -> int:
    return rows * cols * 4 * 9
...
def time_callable(fn: Callable[[], object], warmup: int, iters: int) -> float:
...
def triplet_distance_bwd(
    x: torch.Tensor, y: torch.Tensor, z: torch.Tensor, grad: torch.Tensor
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ap = x - y
    an = x - z
    inv_ap = torch.rsqrt((ap * ap).sum(dim=-1, keepdim=True) + EPS)
    inv_an = torch.rsqrt((an * an).sum(dim=-1, keepdim=True) + EPS)
    gap = ap * inv_ap
    gan = an * inv_an
    dx = grad * (gap - gan)
    dy = -grad * gap
...
def benchmark_torch_compile(
    ...
    x, y, z, grad = make_inputs(rows, cols, device)
    torch._dynamo.reset()
    compiled = torch.compile(triplet_distance_bwd)
    compiled(x, y, z, grad)
    torch.cuda.synchronize()
    avg_ms = time_callable(lambda: compiled(x, y, z, grad), warmup, iters)
    del x, y, z, grad
    torch._dynamo.reset()
...
def main() -> int:
    ...
    print(
        f"Triplet distance backward, rows={args.rows}, warmup={args.warmup}, "
