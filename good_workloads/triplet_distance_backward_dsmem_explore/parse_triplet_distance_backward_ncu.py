...
def summarize(variant: str, path: Path) -> dict[str, float | int | str] | None:
    ...
    modeled_bytes = elements * 4.0 * 9.0
    return {
        "variant": variant,
        "cols": COLS,
        "rows": ROWS,
...
def main() -> int:
    cases = [
        ("block", RESULTS / "ncu_triplet_distance_backward_block.csv"),
        ("cluster", RESULTS / "ncu_triplet_distance_backward_cluster.csv"),
    ]
    rows = []
    ...
    out = RESULTS / "triplet_distance_backward_ncu_summary.csv"
    fieldnames = [
        "variant",
        "cols",
        "rows",
