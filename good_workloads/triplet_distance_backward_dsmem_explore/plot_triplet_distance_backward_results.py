...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot triplet distance backward DSMEM results.")
    parser.add_argument(
        "--input-csv",
        type=Path,
...
def plot_speedup(rows: list[dict[str, str]], out_dir: Path) -> None:
    ...
    ax.set_title("Triplet Distance Backward DSMEM Speedup")
    ax.set_xlabel("N")
    ax.set_ylabel("throughput ratio")
    ax.grid(True, alpha=0.25)
    ax.legend()
    save(fig, out_dir, "triplet_distance_backward_speedup")
...
def plot_runtime(rows: list[dict[str, str]], out_dir: Path) -> None:
    ...
    ax.set_title("Triplet Distance Backward Runtime")
    ax.set_xlabel("N")
    ax.set_ylabel("runtime (ms)")
    ax.grid(True, alpha=0.25)
    ax.legend()
    save(fig, out_dir, "triplet_distance_backward_runtime_ms")
...
def plot_gbps(rows: list[dict[str, str]], out_dir: Path) -> None:
    ...
    ax.plot(xs, [float(row["torch_gbps"]) for row in rows],
            marker="o", label="torch.compile")
    ax.set_title("Triplet Distance Backward Modeled Throughput")
    ax.set_xlabel("N")
    ax.set_ylabel("modeled GB/s")
    ax.grid(True, alpha=0.25)
    ax.legend()
    save(fig, out_dir, "triplet_distance_backward_gbps")


def plot_ncu(out_dir: Path) -> None:
    path = ROOT / "results" / "triplet_distance_backward_ncu_summary.csv"
    if not path.exists():
        return
    with path.open(newline="") as f:
        rows = list(csv.DictReader(f))
    ...
    save(fig, out_dir, "triplet_distance_backward_ncu_metrics")
