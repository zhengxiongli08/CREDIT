...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot LAMB trust-ratio DSMEM results.")
    parser.add_argument(
        "--input-csv",
        type=Path,
...
def read_rows(path: Path) -> list[dict[str, str]]:
    ...
    return [
        row for row in rows
        if row["cluster_ms"] != "n/a" and row["cluster_vs_torch"] != "n/a"
    ]
...
def plot_speedup(rows: list[dict[str, str]], out_dir: Path) -> None:
    ...
    ax.set_title("LAMB Trust-Ratio DSMEM Speedup")
    ax.set_xlabel("N")
    ax.set_ylabel("throughput ratio")
    ax.grid(True, alpha=0.25)
    ax.legend()
    save(fig, out_dir, "lamb_trust_speedup")
...
def plot_runtime(rows: list[dict[str, str]], out_dir: Path) -> None:
    ...
    ax.set_title("LAMB Trust-Ratio Runtime")
    ax.set_xlabel("N")
    ax.set_ylabel("runtime (ms)")
    ax.grid(True, alpha=0.25)
    ax.legend()
    save(fig, out_dir, "lamb_trust_runtime_ms")
...
def plot_gbps(rows: list[dict[str, str]], out_dir: Path) -> None:
    ...
    ax.plot(xs, [float(row["torch_gbps"]) for row in rows],
            marker="o", label="torch.compile")
    ax.set_title("LAMB Trust-Ratio Modeled Throughput")
    ax.set_xlabel("N")
    ax.set_ylabel("modeled GB/s")
    ax.grid(True, alpha=0.25)
    ax.legend()
    save(fig, out_dir, "lamb_trust_gbps")


def plot_ncu(out_dir: Path) -> None:
    path = ROOT / "results" / "lamb_trust_ncu_summary.csv"
    if not path.exists():
        return
    with path.open(newline="") as f:
        rows = list(csv.DictReader(f))
    ...
    save(fig, out_dir, "lamb_trust_ncu_metrics")
