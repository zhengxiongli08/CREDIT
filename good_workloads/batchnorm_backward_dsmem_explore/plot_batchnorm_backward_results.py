...
DEFAULT_CSV = ROOT / "results" / "batchnorm_backward_torch_compare.csv"
DEFAULT_NCU_CSV = ROOT / "results" / "batchnorm_backward_ncu_summary.csv"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot BatchNorm backward DSMEM results.")
...
def plot_speedup(rows: list[dict[str, str]], out_dir: Path) -> None:
    rows = sorted(rows, key=lambda row: int(row["elems_per_channel"]))
    labels = [row["elems_per_channel"] for row in rows]
    ...
    ax.set_title("BatchNorm backward dx: speedup relative to torch.compile")
    ax.set_ylabel("speedup")
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.set_xlabel("elements per channel")
    ax.set_ylim(0, max(max(block), max(cluster), 1.0) * 1.25)
    ax.grid(axis="y", alpha=0.25)
    ax.legend(ncols=3, fontsize=9)
    fig.savefig(out_dir / "batchnorm_backward_speedup_vs_torch.png", dpi=220)
    fig.savefig(out_dir / "batchnorm_backward_speedup_vs_torch.pdf")
    plt.close(fig)
...
def plot_lines(rows: list[dict[str, str]], out_dir: Path, metric: str, ylabel: str, filename: str) -> None:
    ...
    ax.set_title(f"BatchNorm backward dx: {ylabel}")
...
def plot_ncu(path: Path, out_dir: Path) -> None:
    ...
    for ax, (metric, title) in zip(axes, metrics):
        ...
        ax.set_title(title)
        ax.grid(axis="y", alpha=0.25)
    fig.savefig(out_dir / "batchnorm_backward_ncu_metrics.png", dpi=220)
    fig.savefig(out_dir / "batchnorm_backward_ncu_metrics.pdf")
    plt.close(fig)
...
def main() -> int:
    ...
    plot_lines(rows, args.out_dir, "gbps", "GB/s, higher is better", "batchnorm_backward_gbps.png")
    plot_lines(rows, args.out_dir, "ms", "ms, lower is better", "batchnorm_backward_runtime_ms.png")
