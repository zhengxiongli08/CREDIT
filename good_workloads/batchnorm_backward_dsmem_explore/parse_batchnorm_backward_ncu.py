...
ELEMS_PER_CHANNEL = int(os.environ.get("ELEMS_PER_CHANNEL", "65536"))
...
def summarize(variant: str, path: Path) -> dict[str, float | int | str] | None:
    ...
    if not rows:
        return None
    elements = CHANNELS * ELEMS_PER_CHANNEL
    ...
    modeled_bytes = elements * 4.0 * 6.0
    return {
        "variant": variant,
...
def main() -> int:
    cases = [
        ("block", RESULTS / "ncu_batchnorm_backward_block.csv"),
        ("cluster", RESULTS / "ncu_batchnorm_backward_cluster.csv"),
    ]
    rows = []
    ...
    out = RESULTS / "batchnorm_backward_ncu_summary.csv"
    fieldnames = [
        "variant",
        "elems_per_channel",
        "batch_size",
