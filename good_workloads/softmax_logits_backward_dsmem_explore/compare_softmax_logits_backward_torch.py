...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare softmax logits backward DSMEM kernels with torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "softmax_logits_backward_torch_compare.csv",
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_softmax_logits_backward.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def softmax_logits_backward_dx(logits: torch.Tensor, dy: torch.Tensor) -> torch.Tensor:
    probs = torch.softmax(logits, dim=-1)
    dot = (probs * dy).sum(dim=-1, keepdim=True)
    return probs * (dy - dot)
...
def benchmark_torch_compile(
    ...
    logits, dy = make_inputs(rows, cols, device)
    torch._dynamo.reset()
    compiled = torch.compile(softmax_logits_backward_dx)
    compiled(logits, dy)
    torch.cuda.synchronize()
    avg_ms = time_callable(lambda: compiled(logits, dy), warmup, iters)
    del logits, dy
    torch._dynamo.reset()
...
def main() -> int:
    ...
    print(
        f"softmax backward from logits, rows={args.rows}, warmup={args.warmup}, "
