#!/usr/bin/env python3
from __future__ import annotations

import csv
from pathlib import Path


ROOT = Path(__file__).resolve().parent
RESULTS = ROOT / "results"
BATCH_SIZE = 8192
COLS = 65536


def read_ncu_rows(path: Path) -> list[dict[str, str]]:
    lines = path.read_text().splitlines()
    try:
        start = next(i for i, line in enumerate(lines) if line.startswith('"ID",'))
    except StopIteration:
        return []
    rows = list(csv.DictReader(lines[start:]))
    return [row for row in rows if row.get("ID", "").isdigit()]


def to_float(row: dict[str, str], name: str) -> float:
    value = row.get(name, "")
    return float(value) if value else 0.0


def modeled_bytes(workload: str) -> float:
    elements = BATCH_SIZE * COLS
    if workload in {"softmax", "rmsnorm"}:
        return elements * 4.0 * 4.0
    if workload == "cross_entropy":
        return elements * 4.0 * 2.0 + BATCH_SIZE * 4.0
    raise ValueError(workload)


def summarize(workload: str, variant: str, path: Path) -> dict[str, float | int | str] | None:
    rows = read_ncu_rows(path)
    if not rows:
        return None
    elements = BATCH_SIZE * COLS
    time_ns = sum(to_float(row, "gpu__time_duration.sum") for row in rows)
    l2_bytes = sum(to_float(row, "lts__t_bytes.sum") for row in rows)
    dram_read = sum(to_float(row, "dram__bytes_op_read.sum") for row in rows)
    dram_write = sum(to_float(row, "dram__bytes_op_write.sum") for row in rows)
    return {
        "workload": workload,
        "variant": variant,
        "cols": COLS,
        "batch_size": BATCH_SIZE,
        "profiled_launches": len(rows),
        "time_us": time_ns / 1.0e3,
        "modeled_gbps": modeled_bytes(workload) / (time_ns / 1.0e9) / 1.0e9,
        "l2_bytes_per_element": l2_bytes / elements,
        "dram_bytes_per_element": (dram_read + dram_write) / elements,
        "dram_read_bytes_per_element": dram_read / elements,
        "dram_write_bytes_per_element": dram_write / elements,
    }


def main() -> int:
    cases = [
        ("softmax", "block"),
        ("softmax", "cluster"),
        ("rmsnorm", "block"),
        ("rmsnorm", "cluster"),
        ("cross_entropy", "block"),
        ("cross_entropy", "cluster"),
    ]
    rows = []
    for workload, variant in cases:
        path = RESULTS / f"ncu_{workload}_{variant}.csv"
        if not path.exists():
            continue
        summary = summarize(workload, variant, path)
        if summary is not None:
            rows.append(summary)

    out = RESULTS / "markdown_ncu_summary.csv"
    out.parent.mkdir(parents=True, exist_ok=True)
    fieldnames = [
        "workload",
        "variant",
        "cols",
        "batch_size",
        "profiled_launches",
        "time_us",
        "modeled_gbps",
        "l2_bytes_per_element",
        "dram_bytes_per_element",
        "dram_read_bytes_per_element",
        "dram_write_bytes_per_element",
    ]
    with out.open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(rows)

    print(f"Wrote {out}")
    for row in rows:
        print(
            f"{row['workload']:<14} {row['variant']:<8} "
            f"time_us={row['time_us']:.3f} "
            f"l2_B/elem={row['l2_bytes_per_element']:.3f} "
            f"dram_B/elem={row['dram_bytes_per_element']:.3f}"
        )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
