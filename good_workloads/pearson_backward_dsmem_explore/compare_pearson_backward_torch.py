...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare Pearson-correlation backward DSMEM kernels with torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "pearson_backward_torch_compare.csv",
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_pearson_backward.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def pearson_bwd(
    ...
    cov = (xc * yc).sum(dim=-1, keepdim=True)
    inv_x = torch.rsqrt(var_x + EPS)
    inv_y = torch.rsqrt(var_y + EPS)
    inv_xy = inv_x * inv_y
    coeff_x = cov * inv_x * inv_x * inv_x * inv_y
    coeff_y = cov * inv_x * inv_y * inv_y * inv_y
    dx = grad * (yc * inv_xy - xc * coeff_x)
    dy = grad * (xc * inv_xy - yc * coeff_y)
    return dx, dy
...
def benchmark_torch_compile(
    ...
    x, y, grad = make_inputs(rows, cols, device)
    torch._dynamo.reset()
    compiled = torch.compile(pearson_bwd)
    compiled(x, y, grad)
    torch.cuda.synchronize()
    avg_ms = time_callable(lambda: compiled(x, y, grad), warmup, iters)
    del x, y, grad
    torch._dynamo.reset()
...
def main() -> int:
    ...
    print(
        f"Pearson correlation backward, rows={args.rows}, warmup={args.warmup}, "
