...
constexpr float kClipThreshold = 32.0f;
...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--warmup") {
            ...
            std::cout
                << "Usage: grad_centralized_clip_bench [--csv] [--warmup N] "
...
__host__ __device__ __forceinline__ float update_value(float weight,
                                                       ...
                                                       float mean,
                                                       float scale) {
    const float centered = grad - mean;
    return kMomentumCoeff * momentum + scale * centered + kWeightDecay * weight;
}

template <int BlockThreads>
__global__ void block_clip_read_kernel(const float* __restrict__ weight,
    ...
    Pair mean_stats{0.0f, 0.0f};
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        const float gv = row_valid ? grad[row_base + col] : 0.0f;
        mean_stats.a += gv;
    }
    const Pair row_mean_stats = block_reduce_pair<BlockThreads>(mean_stats);
    const float mean = row_mean_stats.a / static_cast<float>(cols);

    Pair norm_stats{0.0f, 0.0f};
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        const float gv = row_valid ? grad[row_base + col] : 0.0f;
        const float centered = gv - mean;
        norm_stats.a += centered * centered;
    }
    const Pair row_norm_stats = block_reduce_pair<BlockThreads>(norm_stats);
    const float scale =
        fminf(1.0f, kClipThreshold / (sqrtf(row_norm_stats.a) + kEps));
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        const float update =
            update_value(wv, grad[row_base + col], momentum[row_base + col],
                         mean, scale);
        weight_out[row_base + col] = wv - kLearningRate * update;
        momentum_out[row_base + col] = update;
    }
}

template <int BlockThreads>
__global__ void block_clip_staged_kernel(const float* __restrict__ weight,
                                         const float* __restrict__ grad,
                                         const float* __restrict__ momentum,
                                         float* __restrict__ weight_out,
                                         float* __restrict__ momentum_out,
    ...
    Pair norm_stats{0.0f, 0.0f};
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        const float centered = tile_g[col] - mean;
        norm_stats.a += centered * centered;
    }
    const Pair row_norm_stats = block_reduce_pair<BlockThreads>(norm_stats);
    const float scale =
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        const float update =
            update_value(tile_w[col], tile_g[col], tile_m[col], mean, scale);
        weight_out[row_base + col] = tile_w[col] - kLearningRate * update;
        momentum_out[row_base + col] = update;
    }
}

template <int BlockThreads, int MaxClusterSize>
__global__ void cluster_clip_staged_kernel(const float* __restrict__ weight,
                                           const float* __restrict__ grad,
                                           const float* __restrict__ momentum,
                                           float* __restrict__ weight_out,
                                           float* __restrict__ momentum_out,
    ...
    const Pair block_mean_stats = block_reduce_pair<BlockThreads>(mean_stats);
    const Pair row_mean_stats =
    ...
    const Pair block_norm_stats = block_reduce_pair<BlockThreads>(norm_stats);
    const Pair row_norm_stats =
        cluster_reduce_pair<BlockThreads, MaxClusterSize>(cluster,
                                                         block_norm_stats);
    const float scale =
        fminf(1.0f, kClipThreshold / (sqrtf(row_norm_stats.a) + kEps));
    ...
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        ...
        const float update =
            update_value(tile_w[col], tile_g[col], tile_m[col], mean, scale);
        weight_out[row_base + global_col] =
            tile_w[col] - kLearningRate * update;
...
DeviceInfo query_device_info(const Options& options) {
    ...
    if (info.cluster_launch) {
        ...
        CUDA_CHECK(cudaOccupancyMaxPotentialClusterSize(
            &max_cluster_size,
            cluster_clip_staged_kernel<kBlockThreads, kMaxClusterSize>,
            &config));
...
VariantResult benchmark_block_read(const Options& options,
    ...
    auto launch = [&]() {
        block_clip_read_kernel<kBlockThreads>
...
VariantResult benchmark_block_staged(const Options& options,
    ...
    CUDA_CHECK(cudaFuncSetAttribute(
        block_clip_staged_kernel<kBlockThreads>,
...
void verify_output_samples(const float* d_weight,
    ...
    for (int row : sample_rows) {
        ...
        for (int col = 0; col < cols; ++col) {
            grad_sum += static_cast<double>(h_grad[col]);
        }
        const double mean = grad_sum / static_cast<double>(cols);
        double centered_sumsq = 0.0;
        for (int col = 0; col < cols; ++col) {
            const double centered = static_cast<double>(h_grad[col]) - mean;
            centered_sumsq += centered * centered;
        }
        const double scale =
            std::min(1.0, static_cast<double>(kClipThreshold) /
                              (std::sqrt(centered_sumsq) + kEps));
        ...
        for (int col : sample_cols) {
            ...
            const double ref_weight =
                wv - static_cast<double>(kLearningRate) * ref_momentum;
            float got = 0.0f;
