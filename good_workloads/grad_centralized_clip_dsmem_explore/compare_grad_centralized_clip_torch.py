...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare gradient-centralized clipped momentum DSMEM kernels "
            "with torch.compile."
        )
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "grad_centralized_clip_torch_compare.csv",
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_grad_centralized_clip.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def make_inputs(
    ...
    momentum = torch.empty((rows, cols), device=device, dtype=torch.float32)
    weight.uniform_(-1.0, 1.0)
    grad.uniform_(-1.0, 1.0)
    momentum.uniform_(-1.0, 1.0)
    return weight, grad, momentum


def modeled_bytes(rows: int, cols: int) -> int:
    return rows * cols * 4 * 7
...
def grad_centralized_clip_update(
    weight: torch.Tensor, grad: torch.Tensor, momentum: torch.Tensor
) -> tuple[torch.Tensor, torch.Tensor]:
    mean = grad.mean(dim=-1, keepdim=True)
    centered = grad - mean
    norm = torch.sqrt((centered * centered).sum(dim=-1, keepdim=True))
    scale = torch.clamp(CLIP_THRESHOLD / (norm + EPS), max=1.0)
    update = MOMENTUM * momentum + scale * centered + WEIGHT_DECAY * weight
    weight_out = weight - LEARNING_RATE * update
    return weight_out, update
...
def benchmark_torch_compile(
    ...
    weight, grad, momentum = make_inputs(rows, cols, device)
    torch._dynamo.reset()
    compiled = torch.compile(grad_centralized_clip_update)
    compiled(weight, grad, momentum)
    torch.cuda.synchronize()
    avg_ms = time_callable(lambda: compiled(weight, grad, momentum), warmup, iters)
    del weight, grad, momentum
    torch._dynamo.reset()
...
def main() -> int:
    ...
    print(
        f"Gradient-centralized clipped momentum update, rows={args.rows}, "
