...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare row-wise min-max normalization DSMEM kernels with torch.compile."
    )
...
def minmax_norm(x: torch.Tensor) -> torch.Tensor:
    lo = torch.amin(x, dim=-1, keepdim=True)
    hi = torch.amax(x, dim=-1, keepdim=True)
...
def benchmark_torch_compile(
    ...
    torch._dynamo.reset()
    compiled = torch.compile(minmax_norm)
    compiled(x)
    torch.cuda.synchronize()
...
def main() -> int:
    ...
    print(
        f"Row-wise min-max normalization, rows={args.rows}, "
