...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--warmup") {
            ...
            std::cout
                << "Usage: minmax_norm_bench [--csv] [--warmup N] "
...
double modeled_bytes(int rows, int cols) {
    return static_cast<double>(rows) * cols * sizeof(float) * 3.0;
}
...
template <int Width>
__device__ __forceinline__ MinMax subwarp_reduce_minmax(MinMax value) {
    for (int offset = Width / 2; offset > 0; offset >>= 1) {
        const float other_lo =
            __shfl_xor_sync(0xffffffffu, value.lo, offset, Width);
        const float other_hi =
            __shfl_xor_sync(0xffffffffu, value.hi, offset, Width);
        value.lo = fminf(value.lo, other_lo);
        value.hi = fmaxf(value.hi, other_hi);
    }
    return value;
}

template <int BlockThreads>
__device__ __forceinline__ MinMax block_reduce_minmax(MinMax value) {
    constexpr int Warps = BlockThreads / 32;
    __shared__ float warp_lo[Warps];
    __shared__ float warp_hi[Warps];
    __shared__ MinMax broadcast;

    value = subwarp_reduce_minmax<32>(value);
    if ((threadIdx.x & 31) == 0) {
        const int warp = threadIdx.x >> 5;
        warp_lo[warp] = value.lo;
        warp_hi[warp] = value.hi;
    }
    __syncthreads();

    MinMax reduced{INFINITY, -INFINITY};
    if (threadIdx.x < Warps) {
        reduced.lo = warp_lo[threadIdx.x];
        reduced.hi = warp_hi[threadIdx.x];
    }
    if (threadIdx.x < 32) {
        reduced = subwarp_reduce_minmax<32>(reduced);
    }
    if (threadIdx.x == 0) {
        broadcast = reduced;
    }
...
__device__ __forceinline__ MinMax cluster_reduce_minmax(cg::cluster_group cluster,
                                                         MinMax block_value) {
    __shared__ float cluster_lo[MaxClusterSize];
    __shared__ float cluster_hi[MaxClusterSize];
    __shared__ MinMax broadcast;
    ...
    if (threadIdx.x < blocks_per_cluster) {
        float* remote_lo =
            cluster.map_shared_rank(&cluster_lo[block_rank], threadIdx.x);
        float* remote_hi =
            cluster.map_shared_rank(&cluster_hi[block_rank], threadIdx.x);
        *remote_lo = block_value.lo;
        *remote_hi = block_value.hi;
    }
    cluster.sync();

    MinMax reduced{INFINITY, -INFINITY};
    if (threadIdx.x < blocks_per_cluster) {
        reduced.lo = cluster_lo[threadIdx.x];
        reduced.hi = cluster_hi[threadIdx.x];
    }
    if (threadIdx.x < 32) {
        reduced = subwarp_reduce_minmax<32>(reduced);
    }
...
__host__ __device__ __forceinline__ float normalize_value(float value, MinMax row) {
    const float denom = row.hi - row.lo + kEps;
    return (value - row.lo) / denom;
}
...
__global__ void block_norm_read_kernel(const float* __restrict__ input,
    ...
    MinMax local{INFINITY, -INFINITY};
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        const float value = row_valid ? input[row_base + col] : 0.0f;
        local.lo = fminf(local.lo, value);
        local.hi = fmaxf(local.hi, value);
    }
    const MinMax row_minmax = block_reduce_minmax<BlockThreads>(local);
    if (!row_valid) {
        return;
    }
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        output[row_base + col] =
            normalize_value(input[row_base + col], row_minmax);
    }
}
...
__global__ void block_norm_staged_kernel(const float* __restrict__ input,
    ...
    MinMax local{INFINITY, -INFINITY};
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        local.lo = fminf(local.lo, tile[col]);
        local.hi = fmaxf(local.hi, tile[col]);
    }
    const MinMax row_minmax = block_reduce_minmax<BlockThreads>(local);
    if (!row_valid) {
        return;
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        output[row_base + col] = normalize_value(tile[col], row_minmax);
    }
}
...
__global__ void cluster_norm_staged_kernel(const float* __restrict__ input,
    ...
    const MinMax block_minmax = block_reduce_minmax<BlockThreads>(local);
    const MinMax row_minmax =
        cluster_reduce_minmax<BlockThreads, MaxClusterSize>(cluster, block_minmax);
    if (!row_valid) {
        return;
    }
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        output[row_base + slice_start + col] =
            normalize_value(tile[col], row_minmax);
    }
    cluster.sync();
}
...
VariantResult benchmark_best_block(const Options& options,
    ...
    best = pick_better(best,
                       benchmark_block_read(options, d_input, d_output, rows,
                                            cols));
    best = pick_better(best,
...
void verify_output_samples(const float* d_input,
    ...
    for (int row : sample_rows) {
        ...
        CUDA_CHECK(cudaMemcpy(h_output.data(), d_output + row_base,
                              static_cast<std::size_t>(cols) * sizeof(float),
                              cudaMemcpyDeviceToHost));
        MinMax ref{INFINITY, -INFINITY};
        for (int col = 0; col < cols; ++col) {
            ref.lo = std::min(ref.lo, h_input[col]);
            ref.hi = std::max(ref.hi, h_input[col]);
        }
        for (int col : sample_cols) {
            const float expected = normalize_value(h_input[col], ref);
            const float err = std::fabs(h_output[col] - expected);
            if (err > 1.0e-5f + 1.0e-5f * std::fabs(expected)) {
...
void verify_selected(const Options& options,
    ...
    if (block.valid && options.profile_variant != "cluster") {
        if (block.name == "block_smem") {
            benchmark_block_staged(one_run, device, d_input, d_output, rows, cols);
        } else {
            benchmark_block_read(one_run, d_input, d_output, rows, cols);
        }
        verify_output_samples(d_input, d_output, rows, cols, "block");
    }
    if (cluster.valid && options.profile_variant != "block") {
        benchmark_cluster(one_run, device, d_input, d_output, rows, cols);
...
void run_shape(int cols, const Options& options, const DeviceInfo& device) {
    ...
    if (options.profile_variant != "cluster") {
        block = benchmark_best_block(options, device, d_input, d_output, rows,
                                     cols);
    }
    if (options.profile_variant != "block") {
        cluster =
            benchmark_cluster(options, device, d_input, d_output, rows, cols);
    }
    verify_selected(options, device, block, cluster, d_input, d_output, rows,
                    cols);
