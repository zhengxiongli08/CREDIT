...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare row-wise top-4 DSMEM kernels with torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "topk_torch_compare.csv",
    )
...
def modeled_bytes(rows: int, cols: int) -> int:
    return rows * cols * 4 + rows * TOP_K * 8
...
def time_callable(fn: Callable[[], tuple[torch.Tensor, torch.Tensor]], warmup: int, iters: int) -> float:
...
def row_topk(x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    result = torch.topk(x, TOP_K, dim=-1, largest=True, sorted=True)
...
def benchmark_torch_compile(
    ...
    torch._dynamo.reset()
    compiled = torch.compile(row_topk)
    compiled(x)
    torch.cuda.synchronize()
...
def main() -> int:
    ...
    print(
        f"Row-wise top-{TOP_K}, rows={args.rows}, warmup={args.warmup}, "
        f"iters={args.iters}, cluster_size={args.cluster_size}"
    )
    print()
    print(
        f"{'N':<10}{'block_GB/s':<12}{'cluster_GB/s':<14}"
        f"{'torch_GB/s':<12}{'cluster/block':<15}{'cluster/torch':<15}"
    )

    combined: list[dict[str, float | int | str]] = []
    for cols in n_values:
        torch_row = benchmark_torch_compile(args.rows, cols, args.warmup, args.iters, device)
        ...
        print(
            f"{cols:<10}{fmt(block_gbps):<12}{fmt(cluster_gbps):<14}"
            f"{fmt(torch_gbps):<12}{fmt(float(cuda_row['cluster_vs_block'])):<15}"
