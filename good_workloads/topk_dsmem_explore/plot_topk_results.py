...
DEFAULT_CSV = ROOT / "results" / "topk_torch_compare.csv"
DEFAULT_NCU_CSV = ROOT / "results" / "topk_ncu_summary.csv"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot row-top4 DSMEM results.")
...
def plot_speedup(rows: list[dict[str, str]], out_dir: Path) -> None:
    ...
    ax.set_title("Row-wise top-4: speedup relative to torch.compile")
    ...
    fig.savefig(out_dir / "topk_speedup_vs_torch.png", dpi=220)
    fig.savefig(out_dir / "topk_speedup_vs_torch.pdf")
    plt.close(fig)
...
def plot_lines(rows: list[dict[str, str]], out_dir: Path, metric: str, ylabel: str, filename: str) -> None:
    ...
    ax.set_title(f"Row-wise top-4: {ylabel}")
...
def plot_ncu(path: Path, out_dir: Path) -> None:
    ...
    for ax, (metric, title) in zip(axes, metrics):
        ...
        ax.set_title(title)
        ax.grid(axis="y", alpha=0.25)
    fig.savefig(out_dir / "topk_ncu_metrics.png", dpi=220)
    fig.savefig(out_dir / "topk_ncu_metrics.pdf")
    plt.close(fig)
...
def main() -> int:
    ...
    plot_lines(rows, args.out_dir, "gbps", "GB/s, higher is better", "topk_gbps.png")
    plot_lines(rows, args.out_dir, "ms", "ms, lower is better", "topk_runtime_ms.png")
