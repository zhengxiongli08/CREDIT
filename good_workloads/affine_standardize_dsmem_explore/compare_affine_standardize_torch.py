...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare row-wise affine standardization DSMEM kernels with torch.compile."
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_affine_standardize.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def benchmark_torch_compile(
    ...
    gamma = make_input(rows, cols, device)
    beta = make_input(rows, cols, device)
    torch._dynamo.reset()
    compiled = torch.compile(affine_standardize)
...
def main() -> int:
    ...
    print(
        f"Row-wise affine standardization, rows={args.rows}, "
