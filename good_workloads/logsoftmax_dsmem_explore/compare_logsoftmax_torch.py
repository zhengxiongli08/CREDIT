...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare log_softmax CUDA DSMEM kernels against torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "logsoftmax_torch_compare.csv",
    )
...
def benchmark_torch_compile(
    ...
    torch._dynamo.reset()
    compiled = torch.compile(lambda inp: F.log_softmax(inp, dim=-1))
    compiled(x)
    torch.cuda.synchronize()
...
def main() -> int:
    ...
    print(
        f"log_softmax comparison, warmup={args.warmup}, iters={args.iters}, "
