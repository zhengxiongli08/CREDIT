...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare BatchNorm training-forward DSMEM kernels with torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "batchnorm_torch_compare.csv",
    )
...
def make_affine(channels: int, device: torch.device) -> tuple[torch.Tensor, torch.Tensor]:
...
def modeled_bytes(channels: int, elems_per_channel: int) -> int:
    return channels * elems_per_channel * 4 * 3
...
def benchmark_torch_compile(
    ...
    x = make_input(batch_size, channels, spatial, device)
    gamma, beta = make_affine(channels, device)
    torch._dynamo.reset()
    compiled = torch.compile(batchnorm_training_forward)
...
def main() -> int:
    ...
    print(
        f"BatchNorm training forward, batch={args.batch_size}, "
