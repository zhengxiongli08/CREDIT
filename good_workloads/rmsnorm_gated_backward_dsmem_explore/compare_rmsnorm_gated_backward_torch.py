...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare RMSNorm gated backward dx DSMEM kernels with torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "rmsnorm_gated_backward_torch_compare.csv",
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_rmsnorm_gated_backward.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def rmsnorm_gated_bwd_dx(
    ...
    return gated_dy * inv_rms - x * coeff
...
def benchmark_torch_compile(
    ...
    torch._dynamo.reset()
    compiled = torch.compile(rmsnorm_gated_bwd_dx)
    compiled(x, dy, gate)
    torch.cuda.synchronize()
...
def main() -> int:
    ...
    print(
        f"RMSNorm gated backward dx, rows={args.rows}, warmup={args.warmup}, "
