...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare row-wise dynamic quantization DSMEM kernels with torch.compile."
    )
...
def rowwise_quantize(x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    inv_scale = torch.where(max_abs > 0.0, QMAX / max_abs, torch.zeros_like(max_abs))
    q = torch.clamp(torch.round(x * inv_scale), -QMAX, QMAX).to(torch.int8)
...
def benchmark_torch_compile(
    ...
    torch._dynamo.reset()
    compiled = torch.compile(rowwise_quantize)
    compiled(x)
    torch.cuda.synchronize()
...
def main() -> int:
    ...
    print(
        f"Row-wise dynamic quantization, rows={args.rows}, "
