...
def summarize(variant: str, path: Path) -> dict[str, float | int | str] | None:
    ...
    modeled_bytes = elements * 9.0 + ROWS * 4.0
    return {
        "variant": variant,
        "cols": COLS,
        "rows": ROWS,
...
def main() -> int:
    cases = [
        ("block", RESULTS / "ncu_rowwise_quant_block.csv"),
        ("cluster", RESULTS / "ncu_rowwise_quant_cluster.csv"),
    ]
    rows = []
    ...
    out = RESULTS / "rowwise_quant_ncu_summary.csv"
    fieldnames = [
        "variant",
        "cols",
        "rows",
