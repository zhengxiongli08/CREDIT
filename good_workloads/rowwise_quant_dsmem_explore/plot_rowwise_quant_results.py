...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot row-wise quantization DSMEM results.")
    parser.add_argument(
        "--input-csv",
        type=Path,
        default=ROOT / "results" / "rowwise_quant_torch_compare.csv",
    )
...
def plot_speedup(rows: list[dict[str, str]], out_dir: Path) -> None:
    ...
    ax.set_title("Row-wise Dynamic Quantization DSMEM Speedup")
    ax.set_xlabel("N")
    ax.set_ylabel("throughput ratio")
    ax.grid(True, alpha=0.25)
    ax.legend()
    save(fig, out_dir, "rowwise_quant_speedup")
...
def plot_runtime(rows: list[dict[str, str]], out_dir: Path) -> None:
    ...
    ax.set_title("Row-wise Dynamic Quantization Runtime")
    ax.set_xlabel("N")
    ax.set_ylabel("runtime (ms)")
    ax.grid(True, alpha=0.25)
    ax.legend()
    save(fig, out_dir, "rowwise_quant_runtime_ms")
...
def plot_gbps(rows: list[dict[str, str]], out_dir: Path) -> None:
    ...
    ax.plot(xs, [float(row["torch_gbps"]) for row in rows],
            marker="o", label="torch.compile")
    ax.set_title("Row-wise Dynamic Quantization Modeled Throughput")
    ax.set_xlabel("N")
    ax.set_ylabel("modeled GB/s")
    ax.grid(True, alpha=0.25)
    ax.legend()
    save(fig, out_dir, "rowwise_quant_gbps")


def plot_ncu(out_dir: Path) -> None:
    path = ROOT / "results" / "rowwise_quant_ncu_summary.csv"
    if not path.exists():
        return
    with path.open(newline="") as f:
        rows = list(csv.DictReader(f))
    ...
    save(fig, out_dir, "rowwise_quant_ncu_metrics")
