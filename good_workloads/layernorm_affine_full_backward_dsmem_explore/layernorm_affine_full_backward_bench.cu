...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--warmup") {
            ...
            std::cout
                << "Usage: layernorm_affine_full_backward_bench [--csv] [--warmup N] "
...
__global__ void init_gamma_kernel(float* gamma, int cols) {
...
__global__ void block_bwd_read_kernel(const float* __restrict__ x,
    ...
    const float mean = x_total.a / cols;
    const float var = fmaxf(x_total.b / cols - mean * mean, 0.0f);
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        const float dyg = dy[row_base + col] * gamma[col];
        grad_stats.a += dyg;
        grad_stats.b += dyg * xhat;
    }
    const Pair grad_total = block_reduce_pair<BlockThreads>(grad_stats);
    const float mean_dyg = grad_total.a / cols;
    const float mean_dyg_xhat = grad_total.b / cols;
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        const float xv = x[row_base + col];
        const float xhat = (xv - mean) * inv_std;
        const float dyv = dy[row_base + col];
        const float dyg = dyv * gamma[col];
        dx[row_base + col] = (dyg - mean_dyg - xhat * mean_dyg_xhat) * inv_std;
        dgamma_partial[row_base + col] = dyv * xhat;
        dbeta_partial[row_base + col] = dyv;
    }
}
...
__global__ void block_bwd_staged_kernel(const float* __restrict__ x,
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        const float xhat = (tile_x[col] - mean) * inv_std;
        const float dyg = tile_dy[col] * tile_gamma[col];
        grad_stats.a += dyg;
        grad_stats.b += dyg * xhat;
    }
    const Pair grad_total = block_reduce_pair<BlockThreads>(grad_stats);
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        const float dyg = tile_dy[col] * tile_gamma[col];
        dx[row_base + col] = (dyg - mean_dyg - xhat * mean_dyg_xhat) * inv_std;
        dgamma_partial[row_base + col] = tile_dy[col] * xhat;
        dbeta_partial[row_base + col] = tile_dy[col];
    }
}
...
__global__ void cluster_bwd_staged_kernel(const float* __restrict__ x,
    ...
    float* tile_gamma = tile + 2 * slice_elems;
    ...
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        ...
        tile_x[col] = x[row_base + global_col];
        tile_dy[col] = dy[row_base + global_col];
        tile_gamma[col] = gamma[global_col];
    }
    __syncthreads();
    ...
    const float mean = row_x.a / cols;
    const float var = fmaxf(row_x.b / cols - mean * mean, 0.0f);
    const float inv_std = rsqrtf(var + eps);
    ...
    const float mean_dyg = row_grad.a / cols;
    const float mean_dyg_xhat = row_grad.b / cols;
    ...
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        ...
        dgamma_partial[row_base + global_col] = tile_dy[col] * xhat;
        dbeta_partial[row_base + global_col] = tile_dy[col];
    }
}
...
VariantResult benchmark_block_read(const Options& options,
                                   ...
                                   const float* d_gamma,
                                   float* d_dx,
                                   float* d_dgamma_partial,
                                   float* d_dbeta_partial,
                                   int rows,
                                   int cols) {
    ...
    auto launch = [&]() {
        block_bwd_read_kernel<kBlockThreads>
            <<<rows, kBlockThreads>>>(d_x, d_dy, d_gamma, d_dx,
                                      d_dgamma_partial, d_dbeta_partial, rows,
                                      cols, kEps);
    };
...
VariantResult benchmark_block_staged(const Options& options,
    ...
    auto launch = [&]() {
        block_bwd_staged_kernel<kBlockThreads>
            <<<rows, kBlockThreads, smem_bytes>>>(d_x, d_dy, d_gamma, d_dx,
                                                  d_dgamma_partial,
                                                  d_dbeta_partial, rows, cols,
                                                  kEps);
    };
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
...
VariantResult benchmark_best_block(const Options& options,
    ...
    best = pick_better(best,
                       benchmark_block_read(options, d_x, d_dy, d_gamma, d_dx,
                                            d_dgamma_partial, d_dbeta_partial,
                                            rows, cols));
    best = pick_better(best,
                       benchmark_block_staged(options, device, d_x, d_dy,
                                              d_gamma, d_dx, d_dgamma_partial,
                                              d_dbeta_partial, rows, cols));
    return best;
}
...
VariantResult benchmark_cluster(const Options& options,
    ...
    auto launch = [&]() {
        ...
        CUDA_CHECK(cudaLaunchKernelEx(
            &config,
            cluster_bwd_staged_kernel<kBlockThreads, kMaxClusterSize>,
            d_x, d_dy, d_gamma, d_dx, d_dgamma_partial, d_dbeta_partial, rows,
            cols, kEps, slice_elems));
    };
    result.valid = true;
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
...
void verify_output_samples(const float* d_x,
                           ...
                           const float* d_gamma,
                           const float* d_dx,
                           const float* d_dgamma_partial,
                           const float* d_dbeta_partial,
                           int rows,
                           int cols,
                           const char* label) {
    ...
    for (int row : sample_rows) {
        ...
        for (int col = 0; col < cols; ++col) {
            const double xhat = (static_cast<double>(h_x[col]) - mean) * inv_std;
            const double dyg =
                static_cast<double>(h_dy[col]) * static_cast<double>(h_gamma[col]);
            sum_dyg += dyg;
            sum_dyg_xhat += dyg * xhat;
        }
        const double mean_dyg = sum_dyg / cols;
        const double mean_dyg_xhat = sum_dyg_xhat / cols;
        ...
        for (int col : sample_cols) {
            ...
            const double ref_dx =
                (dyg - mean_dyg - xhat * mean_dyg_xhat) * inv_std;
            const double ref_dgamma =
                static_cast<double>(h_dy[col]) * xhat;
            const double ref_dbeta = static_cast<double>(h_dy[col]);
            float got_dx = 0.0f;
            float got_dgamma = 0.0f;
            float got_dbeta = 0.0f;
            CUDA_CHECK(cudaMemcpy(&got_dx, d_dx + row_base + col, sizeof(float),
                                  cudaMemcpyDeviceToHost));
            CUDA_CHECK(cudaMemcpy(&got_dgamma, d_dgamma_partial + row_base + col,
                                  sizeof(float), cudaMemcpyDeviceToHost));
            CUDA_CHECK(cudaMemcpy(&got_dbeta, d_dbeta_partial + row_base + col,
                                  sizeof(float), cudaMemcpyDeviceToHost));
            const double err_dx = std::abs(static_cast<double>(got_dx) - ref_dx);
            const double err_dgamma =
                std::abs(static_cast<double>(got_dgamma) - ref_dgamma);
            const double err_dbeta =
                std::abs(static_cast<double>(got_dbeta) - ref_dbeta);
            if (err_dx > 2e-2 + 2e-2 * std::abs(ref_dx)) {
                std::ostringstream oss;
                oss << label << " dx verification failed row=" << row
                    << " col=" << col << " got=" << got_dx << " ref=" << ref_dx
                    << " err=" << err_dx;
                throw std::runtime_error(oss.str());
            }
            if (err_dgamma > 2e-2 + 2e-2 * std::abs(ref_dgamma)) {
                std::ostringstream oss;
                oss << label << " dgamma verification failed row=" << row
                    << " col=" << col << " got=" << got_dgamma
                    << " ref=" << ref_dgamma << " err=" << err_dgamma;
                throw std::runtime_error(oss.str());
            }
            if (err_dbeta > 1e-6 + 1e-6 * std::abs(ref_dbeta)) {
                std::ostringstream oss;
                oss << label << " dbeta verification failed row=" << row
                    << " col=" << col << " got=" << got_dbeta
                    << " ref=" << ref_dbeta << " err=" << err_dbeta;
                throw std::runtime_error(oss.str());
            }
        }
    }
...
void verify_selected(const Options& options,
    ...
    if (block.valid && options.profile_variant != "cluster") {
        if (block.name == "block_smem") {
            benchmark_block_staged(one_run, device, d_x, d_dy, d_gamma, d_dx,
                                   d_dgamma_partial, d_dbeta_partial, rows, cols);
        } else {
            benchmark_block_read(one_run, d_x, d_dy, d_gamma, d_dx,
                                 d_dgamma_partial, d_dbeta_partial, rows, cols);
        }
        verify_output_samples(d_x, d_dy, d_gamma, d_dx, d_dgamma_partial,
                              d_dbeta_partial, rows, cols, "block");
    }
    if (cluster.valid && options.profile_variant != "block") {
        benchmark_cluster(one_run, device, d_x, d_dy, d_gamma, d_dx,
                          d_dgamma_partial, d_dbeta_partial, rows, cols);
        verify_output_samples(d_x, d_dy, d_gamma, d_dx, d_dgamma_partial,
                              d_dbeta_partial, rows, cols, "cluster");
    }
}
...
void run_shape(int cols, const Options& options, const DeviceInfo& device) {
    ...
    float* d_dx = nullptr;
    float* d_dgamma_partial = nullptr;
    float* d_dbeta_partial = nullptr;
    ...
    CUDA_CHECK(cudaMalloc(&d_dgamma_partial, elements * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_dbeta_partial, elements * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_gamma, static_cast<std::size_t>(cols) * sizeof(float)));

    const int init_threads = 256;
    init_x_dy_kernel<<<(elements + init_threads - 1) / init_threads, init_threads>>>(
        d_x, d_dy, elements);
    init_gamma_kernel<<<(cols + init_threads - 1) / init_threads, init_threads>>>(
        d_gamma, cols);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    if (options.profile_variant != "cluster") {
        block = benchmark_best_block(options, device, d_x, d_dy, d_gamma, d_dx,
                                     d_dgamma_partial, d_dbeta_partial, rows, cols);
    }
    if (options.profile_variant != "block") {
        cluster = benchmark_cluster(options, device, d_x, d_dy, d_gamma, d_dx,
                                    d_dgamma_partial, d_dbeta_partial, rows,
                                    cols);
    }
    verify_selected(options, device, block, cluster, d_x, d_dy, d_gamma, d_dx,
                    d_dgamma_partial, d_dbeta_partial, rows, cols);
    ...
    CUDA_CHECK(cudaFree(d_dgamma_partial));
    CUDA_CHECK(cudaFree(d_dbeta_partial));
}
