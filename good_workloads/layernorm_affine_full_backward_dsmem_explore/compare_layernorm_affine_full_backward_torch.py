...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare LayerNorm affine full backward DSMEM kernels with torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "layernorm_affine_full_backward_torch_compare.csv",
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_layernorm_affine_full_backward.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def make_inputs(
    ...
    gamma = torch.empty((cols,), device=device, dtype=torch.float32)
    x.uniform_(-1.0, 1.0)
    dy.uniform_(-1.0, 1.0)
    gamma.uniform_(0.75, 1.25)
    return x, dy, gamma
...
def layernorm_affine_full_bwd(
    x: torch.Tensor, dy: torch.Tensor, gamma: torch.Tensor
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    mean = x.mean(dim=-1, keepdim=True)
    xmu = x - mean
    inv_std = torch.rsqrt((xmu * xmu).mean(dim=-1, keepdim=True) + EPS)
    xhat = xmu * inv_std
    dyg = dy * gamma
    mean_dyg = dyg.mean(dim=-1, keepdim=True)
    mean_dyg_xhat = (dyg * xhat).mean(dim=-1, keepdim=True)
    dx = (dyg - mean_dyg - xhat * mean_dyg_xhat) * inv_std
    dgamma_partial = dy * xhat
    dbeta_partial = dy + 0.0
    return dx, dgamma_partial, dbeta_partial
...
def benchmark_torch_compile(
    ...
    x, dy, gamma = make_inputs(rows, cols, device)
    torch._dynamo.reset()
    compiled = torch.compile(layernorm_affine_full_bwd)
    compiled(x, dy, gamma)
    torch.cuda.synchronize()
...
def main() -> int:
    ...
    print(
        f"LayerNorm affine full backward partials, rows={args.rows}, warmup={args.warmup}, "
