...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare RMSNorm gated full backward DSMEM kernels with torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "rmsnorm_gated_full_backward_torch_compare.csv",
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_rmsnorm_gated_full_backward.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def make_inputs(
    ...
    gate = torch.empty((rows, cols), device=device, dtype=torch.float32)
    x.uniform_(-1.0, 1.0)
    dy.uniform_(-1.0, 1.0)
    gate.uniform_(0.5, 1.5)
    return x, dy, gate


def modeled_bytes(rows: int, cols: int) -> int:
    return rows * cols * 4 * 8
...
def rmsnorm_gated_full_bwd(
    x: torch.Tensor, dy: torch.Tensor, gate: torch.Tensor
) -> tuple[torch.Tensor, torch.Tensor]:
    inv_rms = torch.rsqrt((x * x).mean(dim=-1, keepdim=True) + EPS)
    gated_dy = dy * gate
    dot = (gated_dy * x).sum(dim=-1, keepdim=True)
    coeff = dot * inv_rms * inv_rms * inv_rms / x.shape[-1]
    dx = gated_dy * inv_rms - x * coeff
    dgate = dy * x * inv_rms
    return dx, dgate
...
def benchmark_torch_compile(
    ...
    x, dy, gate = make_inputs(rows, cols, device)
    torch._dynamo.reset()
    compiled = torch.compile(rmsnorm_gated_full_bwd)
    compiled(x, dy, gate)
    torch.cuda.synchronize()
    avg_ms = time_callable(lambda: compiled(x, dy, gate), warmup, iters)
    del x, dy, gate
    torch._dynamo.reset()
...
def main() -> int:
    ...
    print(
        f"RMSNorm gated full backward, rows={args.rows}, warmup={args.warmup}, "
