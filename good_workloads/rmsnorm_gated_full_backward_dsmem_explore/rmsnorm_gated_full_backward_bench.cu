...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--warmup") {
            ...
            std::cout
                << "Usage: rmsnorm_gated_full_backward_bench [--csv] [--warmup N] "
...
__global__ void init_inputs_kernel(float* x,
                                   float* dy,
                                   float* gate,
                                   std::size_t elements) {
    ...
    gate[idx] = 0.5f + static_cast<float>(c & 2047u) * (1.0f / 2048.0f);
}
...
__global__ void block_bwd_read_kernel(const float* __restrict__ x,
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        const float dyv = row_valid ? dy[row_base + col] : 0.0f;
        const float gatev = row_valid ? gate[row_base + col] : 0.0f;
        stats.a += xv * xv;
        stats.b += dyv * gatev * xv;
    }
    const Pair row_stats = block_reduce_pair<BlockThreads>(stats);
    const float inv_rms = rsqrtf(row_stats.a / static_cast<float>(cols) + eps);
    const float coeff =
        row_stats.b * inv_rms * inv_rms * inv_rms / static_cast<float>(cols);
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        const float gatev = gate[row_base + col];
        dx[row_base + col] = dyv * gatev * inv_rms - xv * coeff;
        dgate[row_base + col] = dyv * xv * inv_rms;
    }
}
...
__global__ void block_bwd_staged_kernel(const float* __restrict__ x,
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        tile_gate[col] = row_valid ? gate[row_base + col] : 0.0f;
    }
    __syncthreads();
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        const float dyv = tile_dy[col];
        const float gatev = tile_gate[col];
        stats.a += xv * xv;
        stats.b += dyv * gatev * xv;
    }
    const Pair row_stats = block_reduce_pair<BlockThreads>(stats);
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        dx[row_base + col] =
            tile_dy[col] * tile_gate[col] * inv_rms - tile_x[col] * coeff;
        dgate[row_base + col] = tile_dy[col] * tile_x[col] * inv_rms;
    }
}
...
__global__ void cluster_bwd_staged_kernel(const float* __restrict__ x,
    ...
    float* tile_gate = tile + 2 * slice_elems;
    ...
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        ...
        tile_dy[col] = dy[row_base + slice_start + col];
        tile_gate[col] = gate[row_base + slice_start + col];
    }
    __syncthreads();
    ...
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        ...
        dx[row_base + global_col] =
            tile_dy[col] * tile_gate[col] * inv_rms - tile_x[col] * coeff;
        dgate[row_base + global_col] = tile_dy[col] * tile_x[col] * inv_rms;
    }
}
...
VariantResult benchmark_block_read(const Options& options,
    ...
    auto launch = [&]() {
        block_bwd_read_kernel<kBlockThreads>
            <<<rows, kBlockThreads>>>(d_x, d_dy, d_gate, d_dx, d_dgate, rows,
                                      cols, kEps);
    };
...
VariantResult benchmark_block_staged(const Options& options,
    ...
    auto launch = [&]() {
        block_bwd_staged_kernel<kBlockThreads>
            <<<rows, kBlockThreads, smem_bytes>>>(d_x, d_dy, d_gate, d_dx,
                                                  d_dgate, rows, cols, kEps);
    };
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
    return result;
...
VariantResult benchmark_best_block(const Options& options,
    ...
    best = pick_better(best,
                       benchmark_block_read(options, d_x, d_dy, d_gate, d_dx,
                                            d_dgate, rows, cols));
    best = pick_better(best,
                       benchmark_block_staged(options, device, d_x, d_dy,
                                              d_gate, d_dx, d_dgate, rows,
                                              cols));
    return best;
}
...
VariantResult benchmark_cluster(const Options& options,
    ...
    auto launch = [&]() {
        ...
        CUDA_CHECK(cudaLaunchKernelEx(
            &config,
            cluster_bwd_staged_kernel<kBlockThreads, kMaxClusterSize>,
            d_x, d_dy, d_gate, d_dx, d_dgate, rows, cols, kEps, slice_elems));
    };
    result.valid = true;
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
...
void verify_output_samples(const float* d_x,
    ...
    for (int row : sample_rows) {
        ...
        CUDA_CHECK(cudaMemcpy(h_dy.data(), d_dy + row_base,
                              static_cast<std::size_t>(cols) * sizeof(float),
                              cudaMemcpyDeviceToHost));
        CUDA_CHECK(cudaMemcpy(h_gate.data(), d_gate + row_base,
                              static_cast<std::size_t>(cols) * sizeof(float),
                              cudaMemcpyDeviceToHost));
        ...
        for (int col = 0; col < cols; ++col) {
            ...
            const double dyv = static_cast<double>(h_dy[col]);
            const double gatev = static_cast<double>(h_gate[col]);
            sumsq += xv * xv;
            dot += dyv * gatev * xv;
        }
        const double inv_rms = 1.0 / std::sqrt(sumsq / cols + kEps);
        const double coeff = dot * inv_rms * inv_rms * inv_rms / cols;

        for (int col : sample_cols) {
            const double ref_dx = static_cast<double>(h_dy[col]) *
                                      static_cast<double>(h_gate[col]) *
                                      inv_rms -
                                  static_cast<double>(h_x[col]) * coeff;
            const double ref_dgate =
                static_cast<double>(h_dy[col]) * static_cast<double>(h_x[col]) *
                inv_rms;
            float got = 0.0f;
            CUDA_CHECK(cudaMemcpy(&got, d_dx + row_base + col, sizeof(float),
                                  cudaMemcpyDeviceToHost));
            const double err = std::abs(static_cast<double>(got) - ref_dx);
            if (err > 2e-2 + 2e-2 * std::abs(ref_dx)) {
                std::ostringstream oss;
                oss << label << " dx verification failed row=" << row
                    << " col=" << col << " got=" << got << " ref=" << ref_dx
                    << " err=" << err;
                throw std::runtime_error(oss.str());
            }
            CUDA_CHECK(cudaMemcpy(&got, d_dgate + row_base + col, sizeof(float),
                                  cudaMemcpyDeviceToHost));
            const double dgate_err =
                std::abs(static_cast<double>(got) - ref_dgate);
            if (dgate_err > 2e-2 + 2e-2 * std::abs(ref_dgate)) {
                std::ostringstream oss;
                oss << label << " dgate verification failed row=" << row
                    << " col=" << col << " got=" << got
                    << " ref=" << ref_dgate << " err=" << dgate_err;
                throw std::runtime_error(oss.str());
            }
        }
    }
...
void verify_selected(const Options& options,
    ...
    if (block.valid && options.profile_variant != "cluster") {
        if (block.name == "block_smem") {
            benchmark_block_staged(one_run, device, d_x, d_dy, d_gate, d_dx,
                                   d_dgate, rows, cols);
        } else {
            benchmark_block_read(one_run, d_x, d_dy, d_gate, d_dx, d_dgate,
                                 rows, cols);
        }
        verify_output_samples(d_x, d_dy, d_gate, d_dx, d_dgate, rows, cols,
                              "block");
    }
    if (cluster.valid && options.profile_variant != "block") {
        benchmark_cluster(one_run, device, d_x, d_dy, d_gate, d_dx, d_dgate,
                          rows, cols);
        verify_output_samples(d_x, d_dy, d_gate, d_dx, d_dgate, rows, cols,
                              "cluster");
    }
...
void run_shape(int cols, const Options& options, const DeviceInfo& device) {
    ...
    if (options.profile_variant != "cluster") {
        block = benchmark_best_block(options, device, d_x, d_dy, d_gate, d_dx,
                                     d_dgate, rows, cols);
    }
    if (options.profile_variant != "block") {
        cluster = benchmark_cluster(options, device, d_x, d_dy, d_gate, d_dx,
                                    d_dgate, rows, cols);
    }
    verify_selected(options, device, block, cluster, d_x, d_dy, d_gate, d_dx,
                    d_dgate, rows, cols);
