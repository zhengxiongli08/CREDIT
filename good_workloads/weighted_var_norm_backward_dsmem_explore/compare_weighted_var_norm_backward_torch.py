...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare row-wise weighted variance normalization backward DSMEM kernels with torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "weighted_var_norm_backward_torch_compare.csv",
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_weighted_var_norm_backward.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def make_inputs(
    ...
    x.uniform_(-2.0, 2.0)
    weights = torch.empty((rows, cols), device=device, dtype=torch.float32)
    weights.uniform_(0.5, 1.5)
    dy = torch.empty((rows, cols), device=device, dtype=torch.float32)
    dy.uniform_(-1.0, 1.0)
    return x, weights, dy


def modeled_bytes(rows: int, cols: int) -> int:
    return rows * cols * 4 * 10
...
def weighted_var_norm_backward(
    x: torch.Tensor, weights: torch.Tensor, dy: torch.Tensor
) -> torch.Tensor:
    sum_w = weights.sum(dim=-1, keepdim=True)
    inv_sum_w = 1.0 / (sum_w + EPS)
    mean = (weights * x).sum(dim=-1, keepdim=True) * inv_sum_w
    diff = x - mean
    var = (weights * diff * diff).sum(dim=-1, keepdim=True) * inv_sum_w
    inv_std = torch.rsqrt(var + EPS)
    sum_dy = dy.sum(dim=-1, keepdim=True)
    sum_dy_centered = (dy * diff).sum(dim=-1, keepdim=True)
    return inv_std * (
        dy
        - weights * inv_sum_w * sum_dy
        - weights * diff * inv_std * inv_std * inv_sum_w * sum_dy_centered
    )
...
def benchmark_torch_compile(
    ...
    x, weights, dy = make_inputs(rows, cols, device)
    torch._dynamo.reset()
    compiled = torch.compile(weighted_var_norm_backward)
    compiled(x, weights, dy)
    torch.cuda.synchronize()
    avg_ms = time_callable(lambda: compiled(x, weights, dy), warmup, iters)
    del x, weights, dy
    torch._dynamo.reset()
...
def main() -> int:
    ...
    print(
        f"Row-wise weighted variance normalization backward, rows={args.rows}, "
