...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--warmup") {
            ...
            std::cout
                << "Usage: geommean_norm_bench [--csv] [--warmup N] "
...
__host__ __device__ __forceinline__ float normalize_value(float value,
                                                          float mean_log_abs) {
    return value / expf(mean_log_abs);
}
...
__global__ void block_norm_read_kernel(const float* __restrict__ input,
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        if (row_valid) {
            local_sum += logf(fabsf(value) + kEps);
        }
    }
    const float row_log_sum = block_reduce_sum<BlockThreads>(local_sum);
    if (!row_valid) {
        return;
    }
    const float mean_log_abs = row_log_sum / cols;
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        output[row_base + col] =
            normalize_value(input[row_base + col], mean_log_abs);
    }
}
...
__global__ void block_norm_staged_kernel(const float* __restrict__ input,
    ...
    float local_sum = 0.0f;
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        local_sum += logf(fabsf(tile[col]) + kEps);
    }
    const float row_log_sum = block_reduce_sum<BlockThreads>(local_sum);
    if (!row_valid) {
        return;
    ...
    const float mean_log_abs = row_log_sum / cols;
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        output[row_base + col] = normalize_value(tile[col], mean_log_abs);
    }
}
...
__global__ void cluster_norm_staged_kernel(const float* __restrict__ input,
    ...
    const float block_log_sum = block_reduce_sum<BlockThreads>(local_sum);
    const float row_log_sum =
        cluster_reduce_sum<BlockThreads, MaxClusterSize>(cluster, block_log_sum);
    if (!row_valid) {
        return;
    }
    const float mean_log_abs = row_log_sum / cols;
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        output[row_base + slice_start + col] =
            normalize_value(tile[col], mean_log_abs);
    }
    cluster.sync();
}
...
void verify_output_samples(const float* d_input,
    ...
    for (int row : sample_rows) {
        ...
        for (int col = 0; col < cols; ++col) {
            log_sum += std::log(std::fabs(static_cast<double>(h_input[col])) +
                                static_cast<double>(kEps));
        }
        const double geom_mean = std::exp(log_sum / cols);
        for (int col : sample_cols) {
            const float expected =
                static_cast<float>(static_cast<double>(h_input[col]) / geom_mean);
