...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare row-wise geometric-mean normalization DSMEM kernels with torch.compile."
    )
...
def geommean_norm(x: torch.Tensor) -> torch.Tensor:
    mean_log = torch.mean(torch.log(torch.abs(x) + EPS), dim=-1, keepdim=True)
    return x / torch.exp(mean_log)
...
def benchmark_torch_compile(
    ...
    torch._dynamo.reset()
    compiled = torch.compile(geommean_norm)
    compiled(x)
    torch.cuda.synchronize()
...
def main() -> int:
    ...
    print(
        f"Row-wise geometric-mean normalization, rows={args.rows}, "
