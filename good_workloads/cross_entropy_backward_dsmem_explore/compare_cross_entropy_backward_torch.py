...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare cross entropy backward DSMEM kernels with torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "cross_entropy_backward_torch_compare.csv",
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_cross_entropy_backward.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def cross_entropy_backward_dx(logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
    probs = torch.softmax(logits, dim=-1)
    updates = torch.full_like(targets[:, None], -1.0, dtype=logits.dtype)
    return probs.scatter_add(1, targets[:, None], updates)
...
def benchmark_torch_compile(
    ...
    logits, targets = make_inputs(rows, cols, device)
    torch._dynamo.reset()
    compiled = torch.compile(cross_entropy_backward_dx)
    compiled(logits, targets)
    torch.cuda.synchronize()
    avg_ms = time_callable(lambda: compiled(logits, targets), warmup, iters)
    del logits, targets
    torch._dynamo.reset()
...
def main() -> int:
    ...
    print(
        f"cross entropy backward from logits, rows={args.rows}, warmup={args.warmup}, "
