...
def summarize(variant: str, path: Path) -> dict[str, float | int | str] | None:
    ...
    modeled_bytes = elements * 4.0 * 4.0
    return {
        "variant": variant,
        "cols": COLS,
        "rows": ROWS,
...
def main() -> int:
    cases = [
        ("block", RESULTS / "ncu_cross_entropy_backward_block.csv"),
        ("cluster", RESULTS / "ncu_cross_entropy_backward_cluster.csv"),
    ]
    rows = []
    ...
    out = RESULTS / "cross_entropy_backward_ncu_summary.csv"
    fieldnames = [
        "variant",
        "cols",
        "rows",
