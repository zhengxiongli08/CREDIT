...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--warmup") {
            ...
            std::cout
                << "Usage: cross_entropy_backward_bench [--csv] [--warmup N] "
...
double modeled_bytes(int rows, int cols) {
    return static_cast<double>(rows) * cols * sizeof(float) * 4.0;
}
...
__global__ void init_input_kernel(float* logits,
    ...
    if (idx < static_cast<std::size_t>(rows)) {
        targets[idx] = static_cast<int>((idx * 977ull + 17ull) %
                                        static_cast<unsigned long long>(cols));
    }
}

template <int BlockThreads>
__global__ void block_ce_bwd_read_kernel(const float* __restrict__ logits,
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        dx[row_base + col] = p - (col == target ? 1.0f : 0.0f);
    }
}

template <int BlockThreads>
__global__ void block_ce_bwd_staged_kernel(const float* __restrict__ logits,
                                           const int* __restrict__ targets,
                                           float* __restrict__ dx,
                                           int rows,
                                           int cols) {
    ...
    const float inv_sum = 1.0f / row_sum;
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        const float p = expf(tile[col] - row_max) * inv_sum;
        dx[row_base + col] = p - (col == target ? 1.0f : 0.0f);
    }
}

template <int BlockThreads, int MaxClusterSize>
__global__ void cluster_ce_bwd_staged_kernel(const float* __restrict__ logits,
                                             const int* __restrict__ targets,
                                             float* __restrict__ dx,
                                             int rows,
                                             int cols,
    ...
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        const int global_col = slice_start + col;
        const float p = expf(tile[col] - row_max) * inv_sum;
        dx[row_base + global_col] = p - (global_col == target ? 1.0f : 0.0f);
    }
}
...
DeviceInfo query_device_info(const Options& options) {
    ...
    if (info.cluster_launch) {
        ...
        CUDA_CHECK(cudaOccupancyMaxPotentialClusterSize(
            &max_cluster_size,
            cluster_ce_bwd_staged_kernel<kBlockThreads, kMaxClusterSize>,
            &config));
...
VariantResult benchmark_block_read(const Options& options,
    ...
    auto launch = [&]() {
        block_ce_bwd_read_kernel<kBlockThreads>
            <<<rows, kBlockThreads>>>(d_logits, d_targets, d_dx, rows, cols);
    };
...
VariantResult benchmark_block_staged(const Options& options,
    ...
    CUDA_CHECK(cudaFuncSetAttribute(
        block_ce_bwd_staged_kernel<kBlockThreads>,
    ...
    auto launch = [&]() {
        block_ce_bwd_staged_kernel<kBlockThreads>
            <<<rows, kBlockThreads, smem_bytes>>>(d_logits, d_targets, d_dx, rows,
                                                  cols);
    };
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
...
VariantResult benchmark_best_block(const Options& options,
    ...
    best = pick_better(best,
                       benchmark_block_read(options, d_logits, d_targets, d_dx,
                                            rows, cols));
    best = pick_better(best,
                       benchmark_block_staged(options, device, d_logits,
                                              d_targets, d_dx, rows, cols));
    return best;
}
...
VariantResult benchmark_cluster(const Options& options,
    ...
    auto launch = [&]() {
        ...
        CUDA_CHECK(cudaLaunchKernelEx(
            &config,
            cluster_ce_bwd_staged_kernel<kBlockThreads, kMaxClusterSize>,
            d_logits, d_targets, d_dx, rows, cols, slice_elems));
    };
    result.valid = true;
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
...
void verify_output_samples(const float* d_logits,
    ...
    for (int row : sample_rows) {
        ...
        int target = 0;
        CUDA_CHECK(cudaMemcpy(&target, d_targets + row, sizeof(int),
                              cudaMemcpyDeviceToHost));
        ...
        const auto sample_cols =
            unique_samples({0, cols / 7, cols / 2, cols - 1, target}, cols);
        for (int col : sample_cols) {
            const double p =
                std::exp(static_cast<double>(h_logits[col]) - row_max) / row_sum;
            const double ref = p - (col == target ? 1.0 : 0.0);
            float got = 0.0f;
...
void verify_selected(const Options& options,
    ...
    if (block.valid && options.profile_variant != "cluster") {
        if (block.name == "block_smem") {
            benchmark_block_staged(one_run, device, d_logits, d_targets, d_dx,
                                   rows, cols);
        } else {
            benchmark_block_read(one_run, d_logits, d_targets, d_dx, rows, cols);
        }
        verify_output_samples(d_logits, d_targets, d_dx, rows, cols, "block");
    }
    if (cluster.valid && options.profile_variant != "block") {
        benchmark_cluster(one_run, device, d_logits, d_targets, d_dx, rows, cols);
        verify_output_samples(d_logits, d_targets, d_dx, rows, cols, "cluster");
    }
}
...
void run_shape(int cols, const Options& options, const DeviceInfo& device) {
    ...
    CUDA_CHECK(cudaMalloc(&d_targets, static_cast<std::size_t>(rows) * sizeof(int)));
    ...
    init_input_kernel<<<(elements + init_threads - 1) / init_threads, init_threads>>>(
        d_logits, d_targets, elements, rows, cols);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    if (options.profile_variant != "cluster") {
        block = benchmark_best_block(options, device, d_logits, d_targets, d_dx,
                                     rows, cols);
    }
    if (options.profile_variant != "block") {
        cluster = benchmark_cluster(options, device, d_logits, d_targets, d_dx,
                                    rows, cols);
    }
    verify_selected(options, device, block, cluster, d_logits, d_targets, d_dx,
                    rows, cols);
