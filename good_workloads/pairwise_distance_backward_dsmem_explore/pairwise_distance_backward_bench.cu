...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--warmup") {
            ...
            std::cout
                << "Usage: pairwise_distance_backward_bench [--csv] [--warmup N] "
...
__global__ void block_bwd_read_kernel(const float* __restrict__ x,
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        const float diff = xv - yv;
        sumsq += diff * diff;
    }
    const float row_sumsq = block_reduce_sum<BlockThreads>(sumsq);
    const float inv_dist = rsqrtf(row_sumsq + eps);
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        const float value = grad_row * (xv - yv) * inv_dist;
        dx[row_base + col] = value;
        dy[row_base + col] = -value;
    }
}
...
__global__ void block_bwd_staged_kernel(const float* __restrict__ x,
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        const float value = grad_row * (tile_x[col] - tile_y[col]) * inv_dist;
        dx[row_base + col] = value;
        dy[row_base + col] = -value;
    }
}
...
__global__ void cluster_bwd_staged_kernel(const float* __restrict__ x,
    ...
    const float block_sumsq = block_reduce_sum<BlockThreads>(sumsq);
    const float row_sumsq =
        cluster_reduce_sum<BlockThreads, MaxClusterSize>(cluster, block_sumsq);
    const float inv_dist = rsqrtf(row_sumsq + eps);
    const float grad_row = row_valid ? grad[row] : 0.0f;
    ...
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        ...
        const float value = grad_row * (tile_x[col] - tile_y[col]) * inv_dist;
        dx[row_base + global_col] = value;
        dy[row_base + global_col] = -value;
    }
}
...
void verify_output_samples(const float* d_x,
    ...
    for (int row : sample_rows) {
        ...
        const double inv_dist = 1.0 / std::sqrt(sumsq + kEps);
        const double grad = static_cast<double>(h_grad[row]);

        for (int col : sample_cols) {
            const double xv = static_cast<double>(h_x[col]);
            const double yv = static_cast<double>(h_y[col]);
            const double ref_dx = grad * (xv - yv) * inv_dist;
            const double ref_dy = -ref_dx;
            float got_dx = 0.0f;
            float got_dy = 0.0f;
