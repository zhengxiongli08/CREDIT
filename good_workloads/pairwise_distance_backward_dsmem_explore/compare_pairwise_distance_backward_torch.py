...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare pairwise-distance backward DSMEM kernels with torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "pairwise_distance_backward_torch_compare.csv",
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_pairwise_distance_backward.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def pairwise_distance_bwd(
    x: torch.Tensor, y: torch.Tensor, grad: torch.Tensor
) -> tuple[torch.Tensor, torch.Tensor]:
    diff = x - y
    inv_dist = torch.rsqrt((diff * diff).sum(dim=-1, keepdim=True) + EPS)
    dx = grad * diff * inv_dist
    dy = -dx
    return dx, dy
...
def benchmark_torch_compile(
    ...
    torch._dynamo.reset()
    compiled = torch.compile(pairwise_distance_bwd)
    compiled(x, y, grad)
    torch.cuda.synchronize()
...
def main() -> int:
    ...
    print(
        f"Pairwise distance backward, rows={args.rows}, warmup={args.warmup}, "
