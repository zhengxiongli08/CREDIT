...
DEFAULT_CSV = ROOT / "results" / "weighted_sum_torch_compare.csv"
DEFAULT_NCU_CSV = ROOT / "results" / "weighted_sum_ncu_summary.csv"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot softmax weighted sum results.")
...
def plot_speedup(rows: list[dict[str, str]], out_dir: Path) -> None:
    ...
    ax.set_title("softmax weighted sum: speedup relative to torch.compile")
    ...
    fig.savefig(out_dir / "weighted_sum_speedup_vs_torch.png", dpi=220)
    fig.savefig(out_dir / "weighted_sum_speedup_vs_torch.pdf")
    plt.close(fig)
...
def plot_lines(rows: list[dict[str, str]], out_dir: Path, metric: str, ylabel: str, filename: str) -> None:
    ...
    ax.set_title(f"softmax weighted sum: {ylabel}")
...
def plot_ncu(path: Path, out_dir: Path) -> None:
    ...
    for ax, (metric, title) in zip(axes, metrics):
        ...
        ax.set_title(title)
        ax.grid(axis="y", alpha=0.25)
    fig.savefig(out_dir / "weighted_sum_ncu_metrics.png", dpi=220)
    fig.savefig(out_dir / "weighted_sum_ncu_metrics.pdf")
    plt.close(fig)
...
def main() -> int:
    ...
    plot_lines(rows, args.out_dir, "gbps", "GB/s, higher is better", "weighted_sum_gbps.png")
    plot_lines(rows, args.out_dir, "ms", "ms, lower is better", "weighted_sum_runtime_ms.png")
