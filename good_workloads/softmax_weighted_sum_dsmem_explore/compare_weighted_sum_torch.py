...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare softmax weighted sum CUDA DSMEM kernels against torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "weighted_sum_torch_compare.csv",
    )
...
def benchmark_torch_compile(
    ...
    scores, values = make_inputs(rows, cols, device)
    torch._dynamo.reset()
    compiled = torch.compile(lambda s, v: (F.softmax(s, dim=-1) * v).sum(dim=-1))
...
def main() -> int:
    ...
    print(
        f"softmax weighted sum comparison, warmup={args.warmup}, iters={args.iters}, "
