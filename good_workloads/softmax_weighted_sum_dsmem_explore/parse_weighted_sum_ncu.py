...
def summarize(variant: str, path: Path) -> dict[str, float | int | str] | None:
    ...
    modeled_bytes = elements * 4.0 * 3.0 + BATCH_SIZE * 4.0
    return {
        "variant": variant,
        "cols": COLS,
...
def main() -> int:
    cases = [
        ("block", RESULTS / "ncu_weighted_sum_block.csv"),
        ("cluster", RESULTS / "ncu_weighted_sum_cluster.csv"),
    ]
    rows = []
    ...
    out = RESULTS / "weighted_sum_ncu_summary.csv"
    fieldnames = [
        "variant",
        "cols",
        "batch_size",
