...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare row-wise weighted variance normalization DSMEM kernels with torch.compile."
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_weighted_var_norm.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def weighted_var_norm(x: torch.Tensor, weights: torch.Tensor) -> torch.Tensor:
    sum_w = weights.sum(dim=-1, keepdim=True)
    mean = (weights * x).sum(dim=-1, keepdim=True) / (sum_w + EPS)
    diff = x - mean
    var = (weights * diff * diff).sum(dim=-1, keepdim=True) / (sum_w + EPS)
    return diff * torch.rsqrt(var + EPS)
...
def benchmark_torch_compile(
    ...
    x, weights = make_inputs(rows, cols, device)
    torch._dynamo.reset()
    compiled = torch.compile(weighted_var_norm)
    compiled(x, weights)
    torch.cuda.synchronize()
    avg_ms = time_callable(lambda: compiled(x, weights), warmup, iters)
    del x, weights
    torch._dynamo.reset()
...
def main() -> int:
    ...
    print(
        f"Row-wise weighted variance normalization, rows={args.rows}, "
