...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare entropy backward DSMEM kernels with torch.compile."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=ROOT / "results" / "entropy_backward_torch_compare.csv",
    )
...
def run_cuda(args: argparse.Namespace) -> list[dict[str, float | int | str]]:
    cmd = [
        "bash",
        str(ROOT / "run_entropy_backward.sh"),
        "--csv",
        "--rows",
        str(args.rows),
        "--n-values",
...
def make_inputs(rows: int, cols: int, device: torch.device) -> torch.Tensor:
...
def entropy_backward_dx(logits: torch.Tensor) -> torch.Tensor:
...
def benchmark_torch_compile(
    ...
    logits = make_inputs(rows, cols, device)
    torch._dynamo.reset()
    compiled = torch.compile(entropy_backward_dx)
    compiled(logits)
    torch.cuda.synchronize()
    avg_ms = time_callable(lambda: compiled(logits), warmup, iters)
    del logits
    torch._dynamo.reset()
...
def main() -> int:
    ...
    print(
        f"entropy backward gradient from logits, rows={args.rows}, warmup={args.warmup}, "
