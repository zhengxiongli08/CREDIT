...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--warmup") {
            ...
            std::cout
                << "Usage: entropy_backward_bench [--csv] [--warmup N] "
...
__global__ void init_input_kernel(float* logits, std::size_t elements) {
...
template <int BlockThreads>
__global__ void block_entropy_bwd_read_kernel(const float* __restrict__ logits,
    ...
    float local_weighted_shift = 0.0f;
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        if (row_valid) {
            const float shift = logits[row_base + col] - row_max;
            local_weighted_shift += expf(shift) * shift;
        }
    }
    const float row_weighted_shift =
        block_reduce_sum<BlockThreads>(local_weighted_shift);

    if (!row_valid) {
        return;
    }
    const float inv_sum = 1.0f / row_sum;
    const float mean_shift = row_weighted_shift * inv_sum;
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        const float shift = logits[row_base + col] - row_max;
        const float p = expf(shift) * inv_sum;
        dx[row_base + col] = p * (mean_shift - shift);
    }
}

template <int BlockThreads>
__global__ void block_entropy_bwd_staged_kernel(const float* __restrict__ logits,
                                           float* __restrict__ dx,
                                           int rows,
                                           int cols) {
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        tile[col] = row_valid ? logits[row_base + col] : -CUDART_INF_F;
    }
    __syncthreads();
    ...
    float local_sum = 0.0f;
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        local_sum += expf(tile[col] - row_max);
    }
    const float row_sum = block_reduce_sum<BlockThreads>(local_sum);

    float local_weighted_shift = 0.0f;
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        const float shift = tile[col] - row_max;
        local_weighted_shift += expf(shift) * shift;
    }
    const float row_weighted_shift =
        block_reduce_sum<BlockThreads>(local_weighted_shift);
...
template <int BlockThreads, int MaxClusterSize>
__global__ void cluster_entropy_bwd_staged_kernel(const float* __restrict__ logits,
                                             float* __restrict__ dx,
                                             int rows,
                                             int cols,
                                             int slice_elems) {
    ...
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        ...
        tile[col] = logits[row_base + global_col];
    }
    __syncthreads();
    ...
    const float block_weighted_shift =
        block_reduce_sum<BlockThreads>(local_weighted_shift);
    const float row_weighted_shift =
        cluster_reduce_sum<BlockThreads, MaxClusterSize>(cluster,
                                                         block_weighted_shift);
    ...
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        ...
        const float shift = tile[col] - row_max;
        const float p = expf(shift) * inv_sum;
        dx[row_base + global_col] = p * (mean_shift - shift);
    }
}
...
DeviceInfo query_device_info(const Options& options) {
    ...
    if (info.cluster_launch) {
        ...
        CUDA_CHECK(cudaOccupancyMaxPotentialClusterSize(
            &max_cluster_size,
            cluster_entropy_bwd_staged_kernel<kBlockThreads, kMaxClusterSize>,
            &config));
...
VariantResult benchmark_block_read(const Options& options,
    ...
    auto launch = [&]() {
        block_entropy_bwd_read_kernel<kBlockThreads>
            <<<rows, kBlockThreads>>>(d_logits, d_dx, rows, cols);
    };
...
VariantResult benchmark_block_staged(const Options& options,
    ...
    CUDA_CHECK(cudaFuncSetAttribute(
        block_entropy_bwd_staged_kernel<kBlockThreads>,
    ...
    auto launch = [&]() {
        block_entropy_bwd_staged_kernel<kBlockThreads>
            <<<rows, kBlockThreads, smem_bytes>>>(d_logits, d_dx, rows, cols);
    };
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
    return result;
...
VariantResult benchmark_best_block(const Options& options,
    ...
    best = pick_better(best,
                       benchmark_block_read(options, d_logits, d_dx,
                                            rows, cols));
    best = pick_better(best,
...
VariantResult benchmark_cluster(const Options& options,
    ...
    auto launch = [&]() {
        ...
        CUDA_CHECK(cudaLaunchKernelEx(
            &config,
            cluster_entropy_bwd_staged_kernel<kBlockThreads, kMaxClusterSize>,
            d_logits, d_dx, rows, cols, slice_elems));
    };
    result.valid = true;
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
...
void verify_output_samples(const float* d_logits,
    ...
    for (int row : sample_rows) {
        ...
        double row_sum = 0.0;
        double weighted_shift = 0.0;
        for (float value : h_logits) {
            const double shift = static_cast<double>(value) - row_max;
            const double ev = std::exp(shift);
            row_sum += ev;
            weighted_shift += ev * shift;
        }
        const double mean_shift = weighted_shift / row_sum;

        const auto sample_cols = unique_samples({0, cols / 7, cols / 2, cols - 1}, cols);
        for (int col : sample_cols) {
            const double shift = static_cast<double>(h_logits[col]) - row_max;
            const double p = std::exp(shift) / row_sum;
            const double ref = p * (mean_shift - shift);
            float got = 0.0f;
...
void verify_selected(const Options& options,
    ...
    if (block.valid && options.profile_variant != "cluster") {
        if (block.name == "block_smem") {
            benchmark_block_staged(one_run, device, d_logits, d_dx,
                                   rows, cols);
        } else {
            benchmark_block_read(one_run, d_logits, d_dx, rows, cols);
        }
        verify_output_samples(d_logits, d_dx, rows, cols, "block");
    }
    if (cluster.valid && options.profile_variant != "block") {
        benchmark_cluster(one_run, device, d_logits, d_dx, rows, cols);
        verify_output_samples(d_logits, d_dx, rows, cols, "cluster");
    }
}
...
void run_shape(int cols, const Options& options, const DeviceInfo& device) {
    ...
    if (options.profile_variant != "cluster") {
        block = benchmark_best_block(options, device, d_logits, d_dx, rows, cols);
    }
    if (options.profile_variant != "block") {
        cluster = benchmark_cluster(options, device, d_logits, d_dx, rows, cols);
    }
    verify_selected(options, device, block, cluster, d_logits, d_dx, rows, cols);
