...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare row-wise mean absolute deviation normalization DSMEM kernels with torch.compile."
    )
...
def mad_norm(x: torch.Tensor) -> torch.Tensor:
    mean = x.mean(dim=-1, keepdim=True)
    mad = torch.mean(torch.abs(x - mean), dim=-1, keepdim=True)
    return (x - mean) / (mad + EPS)
...
def benchmark_torch_compile(
    ...
    torch._dynamo.reset()
    compiled = torch.compile(mad_norm)
    compiled(x)
    torch.cuda.synchronize()
...
def main() -> int:
    ...
    print(
        f"Row-wise mean absolute deviation normalization, rows={args.rows}, "
