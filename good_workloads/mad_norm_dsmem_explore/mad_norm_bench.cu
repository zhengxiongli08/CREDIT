...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--warmup") {
            ...
            std::cout
                << "Usage: mad_norm_bench [--csv] [--warmup N] "
...
__global__ void block_norm_read_kernel(const float* __restrict__ input,
    ...
    float local_absdev = 0.0f;
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        const float value = row_valid ? input[row_base + col] : 0.0f;
        local_absdev += fabsf(value - mean);
    }
    const float row_absdev = block_reduce_sum<BlockThreads>(local_absdev);
    const float mad = row_absdev / static_cast<float>(cols);
    if (!row_valid) {
        return;
    }
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        output[row_base + col] =
            normalize_value(input[row_base + col], mean, mad);
    }
}
...
__global__ void block_norm_staged_kernel(const float* __restrict__ input,
    ...
    float local_absdev = 0.0f;
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        local_absdev += fabsf(tile[col] - mean);
    }
    const float row_absdev = block_reduce_sum<BlockThreads>(local_absdev);
    const float mad = row_absdev / static_cast<float>(cols);
    if (!row_valid) {
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        output[row_base + col] = normalize_value(tile[col], mean, mad);
    }
}
...
__global__ void cluster_norm_staged_kernel(const float* __restrict__ input,
    ...
    const float block_absdev = block_reduce_sum<BlockThreads>(local_absdev);
    const float row_absdev =
        cluster_reduce_sum<BlockThreads, MaxClusterSize>(cluster, block_absdev);
    const float mad = row_absdev / static_cast<float>(cols);
    if (!row_valid) {
        return;
    }
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        output[row_base + slice_start + col] =
            normalize_value(tile[col], mean, mad);
    }
    cluster.sync();
}
...
void verify_output_samples(const float* d_input,
    ...
    for (int row : sample_rows) {
        ...
        for (int col = 0; col < cols; ++col) {
            sum += static_cast<double>(h_input[col]);
        }
        const double mean = sum / cols;
        double absdev = 0.0;
        for (int col = 0; col < cols; ++col) {
            absdev += std::fabs(static_cast<double>(h_input[col]) - mean);
        }
        const double mad = absdev / cols;
