...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare row-wise variance clipping DSMEM kernels with torch.compile."
    )
...
def benchmark_torch_compile(
    ...
    torch._dynamo.reset()
    compiled = torch.compile(varclip_norm)
    compiled(x)
    torch.cuda.synchronize()
...
def main() -> int:
    ...
    print(
        f"Row-wise variance clipping, rows={args.rows}, "
