...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--warmup") {
            ...
            std::cout
                << "Usage: layernorm_backward_bench [--csv] [--warmup N] "
...
__global__ void block_bwd_staged_kernel(const float* __restrict__ x,
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        tile[col] = row_valid ? x[row_base + col] : 0.0f;
    }
    __syncthreads();

    Pair x_stats{0.0f, 0.0f};
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        const float xv = tile[col];
        x_stats.a += xv;
        x_stats.b += xv * xv;
    }
    const Pair x_total = block_reduce_pair<BlockThreads>(x_stats);
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        const float xhat = (tile[col] - mean) * inv_std;
        const float dyg = dy[row_base + col] * gamma[col];
        grad_stats.a += dyg;
        grad_stats.b += dyg * xhat;
    }
...
__global__ void cluster_bwd_staged_kernel(const float* __restrict__ x,
    ...
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        tile[col] = x[row_base + slice_start + col];
    }
    __syncthreads();
    ...
    for (int col = threadIdx.x; col < local_cols; col += BlockThreads) {
        ...
        const float xhat = (tile[col] - mean) * inv_std;
        const float dyg = dy[row_base + global_col] * gamma[global_col];
        grad_stats.a += dyg;
        grad_stats.b += dyg * xhat;
    }
...
VariantResult benchmark_block_read(const Options& options,
    ...
    auto launch = [&]() {
        block_bwd_read_kernel<kBlockThreads>
            <<<rows, kBlockThreads>>>(d_x, d_dy, d_gamma, d_dx, rows, cols, kEps);
    };
...
VariantResult benchmark_block_staged(const Options& options,
    ...
    auto launch = [&]() {
        block_bwd_staged_kernel<kBlockThreads>
            <<<rows, kBlockThreads, smem_bytes>>>(d_x, d_dy, d_gamma, d_dx, rows,
                                                  cols, kEps);
    };
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
...
VariantResult benchmark_cluster(const Options& options,
    ...
    auto launch = [&]() {
        ...
        CUDA_CHECK(cudaLaunchKernelEx(
            &config,
            cluster_bwd_staged_kernel<kBlockThreads, kMaxClusterSize>,
            d_x, d_dy, d_gamma, d_dx, rows, cols, kEps, slice_elems));
    };
    result.valid = true;
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
...
void verify_selected(const Options& options,
    ...
    if (block.valid && options.profile_variant != "cluster") {
        if (block.name == "block_smem") {
            ...
            benchmark_block_read(one_run, d_x, d_dy, d_gamma, d_dx, rows, cols);
        }
        verify_output_samples(d_x, d_dy, d_gamma, d_dx, rows, cols, "block");
    }
    if (cluster.valid && options.profile_variant != "block") {
        benchmark_cluster(one_run, device, d_x, d_dy, d_gamma, d_dx, rows, cols);
        verify_output_samples(d_x, d_dy, d_gamma, d_dx, rows, cols, "cluster");
    }
}
