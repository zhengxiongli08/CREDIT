...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--warmup") {
            ...
            std::cout
                << "Usage: weighted_var_norm_backward_bench [--csv] [--warmup N] "
...
double modeled_bytes(int rows, int cols) {
    return static_cast<double>(rows) * cols * sizeof(float) * 10.0;
}
...
__global__ void init_weight_kernel(float* data, std::size_t elements) {
    const std::size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= elements) {
        return;
    }
    const unsigned a =
        static_cast<unsigned>((idx * 1664525ull + 1013904223ull) >> 16);
    data[idx] = 0.5f + static_cast<float>(a & 4095u) * (1.0f / 4096.0f);
}

__global__ void init_dy_kernel(float* data, std::size_t elements) {
    const std::size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= elements) {
        return;
    }
    ...
    data[idx] = static_cast<float>(a & 4095u) * (1.0f / 2048.0f) - 1.0f;
}

__host__ __device__ __forceinline__ float backward_value(float value,
                                                         float weight,
                                                         float grad,
                                                         float mean,
                                                         float inv_sum_w,
                                                         float inv_std,
                                                         float sum_dy,
                                                         float sum_dy_centered) {
    const float centered = value - mean;
    const float inv_var = inv_std * inv_std;
    return inv_std *
           (grad - weight * inv_sum_w * sum_dy -
            weight * centered * inv_var * inv_sum_w * sum_dy_centered);
}

template <int BlockThreads>
__global__ void block_bwd_read_kernel(const float* __restrict__ input,
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        const float weight = row_valid ? weights[row_base + col] : 0.0f;
        local.sum += weight;
        local.sumsq += weight * value;
    }
    const Pair row_weighted = block_reduce_pair<BlockThreads>(local);
    const float inv_sum_w = 1.0f / (row_weighted.sum + kEps);
    const float mean = row_weighted.sumsq * inv_sum_w;

    Pair local_var{0.0f, 0.0f};
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        const float value = row_valid ? input[row_base + col] : 0.0f;
        const float weight = row_valid ? weights[row_base + col] : 0.0f;
        const float diff = value - mean;
        local_var.sum += weight * diff * diff;
    }
    const Pair row_var_pair = block_reduce_pair<BlockThreads>(local_var);
    const float var = fmaxf(row_var_pair.sum * inv_sum_w, 0.0f);
    const float inv_std = rsqrtf(var + kEps);

    Pair local_grad{0.0f, 0.0f};
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        const float value = row_valid ? input[row_base + col] : 0.0f;
        const float grad = row_valid ? dy[row_base + col] : 0.0f;
        local_grad.sum += grad;
        local_grad.sumsq += grad * (value - mean);
    }
    const Pair row_grad = block_reduce_pair<BlockThreads>(local_grad);
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        output[row_base + col] =
            backward_value(input[row_base + col],
                           weights[row_base + col],
                           dy[row_base + col],
                           mean,
                           inv_sum_w,
                           inv_std,
...
template <int BlockThreads>
__global__ void block_bwd_staged_kernel(const float* __restrict__ input,
                                        const float* __restrict__ weights,
                                        const float* __restrict__ dy,
                                        float* __restrict__ output,
                                        int rows,
    ...
    float* tile_x = tile;
    float* tile_w = tile + cols;
    float* tile_dy = tile + 2 * cols;
    const int row = blockIdx.x;
    const bool row_valid = row < rows;
    const std::size_t row_base = static_cast<std::size_t>(row) * cols;
    ...
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        ...
        tile_w[col] = row_valid ? weights[row_base + col] : 0.0f;
    ...
    Pair local_var{0.0f, 0.0f};
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        const float diff = tile_x[col] - mean;
        local_var.sum += tile_w[col] * diff * diff;
    }
    const Pair row_var_pair = block_reduce_pair<BlockThreads>(local_var);
    const float var = fmaxf(row_var_pair.sum * inv_sum_w, 0.0f);
    const float inv_std = rsqrtf(var + kEps);
    ...
    Pair local_grad{0.0f, 0.0f};
    for (int col = threadIdx.x; col < cols; col += BlockThreads) {
        const float grad = tile_dy[col];
        local_grad.sum += grad;
        local_grad.sumsq += grad * (tile_x[col] - mean);
    }
    const Pair row_grad = block_reduce_pair<BlockThreads>(local_grad);
...
template <int BlockThreads, int MaxClusterSize>
__global__ void cluster_bwd_staged_kernel(const float* __restrict__ input,
                                          const float* __restrict__ weights,
                                          const float* __restrict__ dy,
                                          float* __restrict__ output,
                                          int rows,
    ...
    extern __shared__ float tile[];
    float* tile_x = tile;
    float* tile_w = tile + slice_elems;
    float* tile_dy = tile + 2 * slice_elems;
    ...
    const Pair block_weighted = block_reduce_pair<BlockThreads>(local);
    const Pair row_weighted =
        cluster_reduce_pair<BlockThreads, MaxClusterSize>(cluster,
                                                          block_weighted);
    const float inv_sum_w = 1.0f / (row_weighted.sum + kEps);
    const float mean = row_weighted.sumsq * inv_sum_w;
    ...
    const Pair block_var = block_reduce_pair<BlockThreads>(local_var);
    const Pair row_var_pair =
        cluster_reduce_pair<BlockThreads, MaxClusterSize>(cluster, block_var);
    const float var = fmaxf(row_var_pair.sum * inv_sum_w, 0.0f);
    const float inv_std = rsqrtf(var + kEps);
    ...
    const Pair block_grad = block_reduce_pair<BlockThreads>(local_grad);
    const Pair row_grad =
...
VariantResult benchmark_block_read(const Options& options,
    ...
    auto launch = [&]() {
        block_bwd_read_kernel<kBlockThreads>
            <<<rows, kBlockThreads>>>(d_input, d_weights, d_dy, d_output, rows,
                                      cols);
    };
...
VariantResult benchmark_block_staged(const Options& options,
    ...
    auto launch = [&]() {
        block_bwd_staged_kernel<kBlockThreads>
            <<<rows, kBlockThreads, smem_bytes>>>(d_input, d_weights, d_dy,
                                                  d_output, rows, cols);
    };
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
...
VariantResult benchmark_best_block(const Options& options,
    ...
    best = pick_better(best,
                       benchmark_block_read(options, d_input, d_weights, d_dy,
                                            d_output, rows, cols));
    best = pick_better(best,
                       benchmark_block_staged(options, device, d_input,
                                              d_weights, d_dy, d_output, rows,
                                              cols));
    return best;
}
...
VariantResult benchmark_cluster(const Options& options,
    ...
    auto launch = [&]() {
        ...
        CUDA_CHECK(cudaLaunchKernelEx(
            &config,
            cluster_bwd_staged_kernel<kBlockThreads, kMaxClusterSize>,
            d_input, d_weights, d_dy, d_output, rows, cols, slice_elems));
    };
    result.valid = true;
    result.avg_ms = time_kernel(launch, options.warmup, options.iters);
    result.gbps = throughput_gbps(rows, cols, result.avg_ms);
...
void verify_output_samples(const float* d_input,
    ...
    std::vector<float> h_weights(cols);
    ...
    for (int row : sample_rows) {
        ...
        CUDA_CHECK(cudaMemcpy(h_weights.data(), d_weights + row_base,
                              static_cast<std::size_t>(cols) * sizeof(float),
                              cudaMemcpyDeviceToHost));
        ...
        for (int col = 0; col < cols; ++col) {
            ...
            const double weight = static_cast<double>(h_weights[col]);
            sum_w += weight;
            sum_wx += weight * value;
        }
        const double mean = sum_wx / (sum_w + kEps);
        double weighted_var = 0.0;
        for (int col = 0; col < cols; ++col) {
            const double diff = static_cast<double>(h_input[col]) - mean;
            weighted_var += static_cast<double>(h_weights[col]) * diff * diff;
        }
        const double var = std::max(weighted_var / (sum_w + kEps), 0.0);
        const double inv_std = 1.0 / std::sqrt(var + kEps);
        const double inv_sum_w = 1.0 / (sum_w + kEps);
        double sum_dy = 0.0;
        double sum_dy_centered = 0.0;
        for (int col = 0; col < cols; ++col) {
            const double centered = static_cast<double>(h_input[col]) - mean;
            const double grad = static_cast<double>(h_dy[col]);
            sum_dy += grad;
            sum_dy_centered += grad * centered;
        }
        for (int col : sample_cols) {
            ...
            const double expected =
                inv_std *
                (grad - weight * inv_sum_w * sum_dy -
                 weight * centered * inv_std * inv_std * inv_sum_w *
                     sum_dy_centered);
            const double err = std::fabs(static_cast<double>(h_output[col]) -
                                         expected);
            if (err > 2.0e-4 + 2.0e-4 * std::fabs(expected)) {
...
void verify_selected(const Options& options,
    ...
    if (block.valid && options.profile_variant != "cluster") {
        if (block.name == "block_smem") {
            benchmark_block_staged(one_run, device, d_input, d_weights, d_dy,
                                   d_output, rows, cols);
        } else {
            benchmark_block_read(one_run, d_input, d_weights, d_dy, d_output,
                                 rows, cols);
        }
        verify_output_samples(d_input, d_weights, d_dy, d_output, rows, cols,
                              "block");
    }
    if (cluster.valid && options.profile_variant != "block") {
        benchmark_cluster(one_run, device, d_input, d_weights, d_dy, d_output,
                          rows, cols);
        verify_output_samples(d_input, d_weights, d_dy, d_output, rows, cols,
                              "cluster");
    }
...
void run_shape(int cols, const Options& options, const DeviceInfo& device) {
    ...
    CUDA_CHECK(cudaMalloc(&d_weights, elements * sizeof(float)));
    ...
    init_weight_kernel<<<(elements + init_threads - 1) / init_threads,
                         init_threads>>>(d_weights, elements);
    init_dy_kernel<<<(elements + init_threads - 1) / init_threads,
                     init_threads>>>(d_dy, elements);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    if (options.profile_variant != "cluster") {
        block = benchmark_best_block(options, device, d_input, d_weights, d_dy,
                                     d_output, rows, cols);
    }
    if (options.profile_variant != "block") {
        cluster =
            benchmark_cluster(options, device, d_input, d_weights, d_dy,
                              d_output, rows, cols);
    }
    verify_selected(options, device, block, cluster, d_input, d_weights, d_dy,
                    d_output, rows, cols);
